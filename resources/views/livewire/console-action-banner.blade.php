@php
    /** @var \Illuminate\Support\Collection<int, \App\Models\ConsoleAction> $runs */
    /** @var array<string, array{running: string, completed: string, failed: string, stale: string}> $kindLabels */

    // Pull host name once for `:host` substitution in kind labels. Falls back to
    // a generic placeholder when the subject doesn't expose one (rare).
    $resolveHost = static function (\App\Models\ConsoleAction $run): string {
        $subject = $run->subject;
        if ($subject === null) {
            return 'host';
        }
        if (method_exists($subject, 'name') && is_string($subject->name)) {
            return $subject->name;
        }

        return (string) ($subject->name ?? $subject->primaryDomain()?->hostname ?? 'host');
    };
@endphp

<div wire:key="console-action-banner-root-{{ $this->subjectId }}">
    @if ($busy)
        {{-- Empty hidden poll element — only mounted while at least one run is
             still in-flight, so the component goes quiet once everything's
             terminal and stops hitting the DB on a 4s cadence. --}}
        <div wire:poll.4s="refreshState" class="hidden" aria-hidden="true"></div>
    @endif

    @forelse ($runs as $run)
        @php
            $kind = $kindLabels[$run->kind] ?? null;
            $host = $resolveHost($run);
            $stale = $run->isStale();
        ...
        >
            ...
            <div x-show="expanded" x-cloak class="border-t border-current/15 bg-white/70 px-4 py-3">
                ...
                @else
                    {{-- Black console pane mirrors the existing apply-banner output area.
                         Lines are colored by level so step/warn/error/success scan visually
                         without the operator having to read prefixes. --}}
...
@endforeach</pre>
    ...
    @empty
        {{-- Nothing to show; keep the wrapper element so wire:morph has a stable target. --}}
    @endforelse
</div>
