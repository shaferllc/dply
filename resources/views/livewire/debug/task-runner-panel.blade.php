        ...
        >
            <div class="mx-auto max-w-7xl pointer-events-auto">
                ...
                <button
                    ...
                    class="flex w-full items-center justify-between gap-3 border-t border-x border-zinc-700/60 bg-zinc-900 px-3 py-1.5 text-xs font-mono text-zinc-100 shadow-lg hover:bg-zinc-800 focus:outline-none"
                ...
                >
                    ...
                    <span class="text-zinc-400 text-2xs">
                ...
                <div x-show="expanded" x-cloak
                    ...
                    {{-- Tabs --}}
                    <div class="flex shrink-0 items-center gap-1 px-3 pt-2 text-xs uppercase font-mono tracking-wide">
                        @foreach (['live' => __('Live'), 'recent' => __('Recent'), 'all' => __('All')] as $key => $label)
                            <button type="button"
                                ...
                                {{ $label }}
                                @if ($key === 'live')
                                    <span x-show="liveCount > 0" class="ml-1 inline-flex items-center justify-center rounded-full bg-emerald-500/20 px-1.5 py-0.5 text-3xs text-emerald-300" x-text="liveCount"></span>
                                @endif
                                @if ($key === 'recent' && $recent->isNotEmpty())
                                    <span class="ml-1 inline-flex items-center justify-center rounded-full bg-zinc-700/50 px-1.5 py-0.5 text-3xs text-zinc-300">{{ $recent->count() }}</span>
                                @endif
                            </button>
                        @endforeach
                        <button
                            ...
                            class="ml-auto inline-flex items-center gap-1.5 rounded px-2 py-0.5 text-2xs font-mono uppercase tracking-wide ring-1 ring-inset {{ $mineOnly ? 'bg-emerald-500/15 text-emerald-300 ring-emerald-500/30' : 'bg-zinc-800/60 text-zinc-300 ring-zinc-700' }}"
                        >
                        ...
                        <span class="ml-2 text-2xs text-zinc-500">org · {{ $this->organizationId ?? '—' }}</span>
                    </div>
                    ...
                    <div class="flex flex-1 min-h-0 overflow-hidden border-t border-zinc-800">
                            ...
                            <div x-show="$wire.tab === 'live'">
                                <template x-for="(row, idx) in liveBuffer" :key="row.uid">
                                    <div class="px-3 py-1.5 border-b border-zinc-800/60">
                                        <div class="flex justify-between text-2xs uppercase tracking-wide">
                                            <span class="font-semibold text-emerald-300" x-text="row.kind"></span>
                                            <span class="text-zinc-500" x-text="row.at"></span>
                                        </div>
                                        <pre class="mt-0.5 whitespace-pre-wrap break-all text-xs leading-snug text-zinc-200" x-text="row.preview"></pre>
                                    </div>
                                </template>
                                <p x-show="liveBuffer.length === 0" class="p-3 text-xs text-zinc-500">
                            ...
                            @if ($tab === 'recent' || $tab === 'all')
                                @forelse ($recent as $row)
                                    <button type="button"
                                        ...
                                        <div class="flex items-center justify-between gap-2 text-2xs font-mono">
                                        ...
                                        <div class="mt-0.5 truncate font-mono text-xs text-zinc-200">{{ $row->commandPreview ?: $row->label }}</div>
                                    </button>
                                @empty
                        ...
                        <div class="w-1/2 min-w-0 overflow-y-auto border-l border-zinc-800 bg-black">
                            @if ($detailSource && $detailId)
                                <div class="flex items-center justify-between px-3 py-1.5 text-2xs uppercase tracking-wide text-zinc-400 border-b border-zinc-800">
                                ...
                                @if ($detailOutput === null || trim($detailOutput) === '')
                                    <p class="p-3 text-xs text-zinc-500">{{ __('(no output captured)') }}</p>
                                @else
                                    <pre class="p-3 whitespace-pre-wrap break-all font-mono text-xs leading-snug text-emerald-200">{{ $detailOutput }}</pre>
                                @endif
                            @else
                                <p class="p-3 text-xs text-zinc-500">{{ __('Select a row to view full output.') }}</p>
                            @endif
                        </div>
                    </div>
                </div>
