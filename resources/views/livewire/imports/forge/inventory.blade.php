<div class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    <header class="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div class="space-y-1">
            <h1 class="text-2xl font-semibold text-brand-ink">{{ __('Imports · Laravel Forge') }}</h1>
            <p class="max-w-2xl text-sm leading-relaxed text-brand-moss">
                {{ __('Your existing Forge servers and sites. Click a server to migrate it to a new dply-managed server — we move code, env, databases, scheduled jobs, daemons, and SSL.') }}
            </p>
        </div>
        <div class="flex items-center gap-2">
            @if ($hasCredentials)
                ...
                <x-secondary-button wire:click="refresh" wire:loading.attr="disabled" wire:target="refresh">
                    ...
                    <span wire:loading.remove wire:target="refresh">{{ __('Refresh from Forge') }}</span>
            ...
            <a href="{{ route('credentials.index', ['provider' => 'forge']) }}" wire:navigate>
                <x-secondary-button type="button">
    ...
    @if (! $hasCredentials)
        <section class="dply-card overflow-hidden">
            <div class="space-y-4 p-8 text-center">
                ...
                <div class="space-y-2">
                    <h2 class="text-base font-semibold text-brand-ink">{{ __('Connect Laravel Forge to see your inventory') }}</h2>
                    <p class="mx-auto max-w-md text-sm leading-relaxed text-brand-moss">
                        {{ __('Generate an API token on your Forge profile and connect it here. We will then list your existing servers and sites and let you migrate each one onto a dply-managed server.') }}
                    </p>
                </div>
                <a href="{{ route('credentials.index', ['provider' => 'forge']) }}" wire:navigate
        ...
        <section class="dply-card overflow-hidden">
            <div class="space-y-3 p-8 text-center">
                <p class="text-sm text-brand-moss">
                    {{ __('No Forge servers found yet. We will sync them in the background — click Refresh if the list does not appear within a minute.') }}
                </p>
            </div>
        </section>
    @else
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-3 border-b border-brand-ink/10 bg-brand-cream/40 px-5 py-4">
                        <div class="space-y-1">
                            ...
                            <p class="text-xs text-brand-moss">
                                @if ($server->ip_address) <span class="font-mono">{{ $server->ip_address }}</span> @endif
                                @if ($server->provider_label) · {{ $server->provider_label }} @endif
                                @if ($server->server_type) · {{ $server->server_type }} @endif
