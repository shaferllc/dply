    ...
    @feature('workspace.webserver_config_diff')
        @if ($config_contents !== $config_original_contents)
            <div class="mt-3 flex flex-wrap items-center gap-2">
                <button
                    ...
                    class="inline-flex items-center gap-1 rounded-md border border-violet-200 bg-violet-50 px-2.5 py-1 text-xs font-semibold text-violet-800 hover:bg-violet-100"
                >
                    <x-heroicon-o-arrows-right-left class="h-3 w-3" />
                    {{ $webserverConfigSaveDiffOpen ? __('Hide save diff') : __('Review diff before save') }}
                </button>
                <span class="text-xs text-brand-moss">{{ __('Compares the loaded file against your editor buffer.') }}</span>
            </div>
        @endif
        ...
        @if ($webserverConfigSaveDiffOpen && $webserverConfigSaveDiffText !== null)
            <div class="mt-3 rounded-xl border border-violet-200 bg-violet-50/40 p-3">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-violet-800">{{ __('Pending save diff') }}</p>
        ...
        @if ($webserverConfigDiffText !== null)
            <div class="mt-3 rounded-xl border border-brand-ink/10 bg-white p-3">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <button type="button" wire:click="closeWebserverConfigRevisionDiff" class="text-xs font-medium text-brand-moss hover:text-brand-ink">
                        {{ __('Close diff') }}
                    </button>
                </div>
        ...
        @if ($webserverConfigRevisions->isNotEmpty())
            <div class="mt-3 rounded-xl border border-brand-ink/10 bg-white">
                <div class="flex items-center justify-between border-b border-brand-ink/10 px-3 py-2">
                    ...
                    <button type="button" wire:click="toggleWebserverConfigCompareMode" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">
                        {{ $webserverConfigCompareMode ? __('Cancel compare') : __('Compare two') }}
                    </button>
                </div>

                @if ($webserverConfigDriftDetected)
                    <div class="border-b border-amber-200 bg-amber-50/70 px-3 py-2 text-xs text-amber-900">
                        ...
                        <li class="flex items-start justify-between gap-3 px-3 py-2 {{ $isCurrent ? 'bg-emerald-50/40' : '' }}" wire:key="ws-rev-{{ $rev->id }}">
                            <div class="min-w-0">
                                <p class="font-medium text-brand-ink">
                                    ...
                                    @if ($isCurrent)
                                        <span class="ml-1 rounded-full bg-emerald-50 px-1.5 py-0.5 text-3xs font-semibold uppercase tracking-wide text-emerald-800 ring-1 ring-emerald-200">{{ __('Current') }}</span>
                                    @endif
                                </p>
                                @if ($rev->user)
                                    <p class="text-2xs text-brand-moss">{{ $rev->user->name }}</p>
                                @endif
                                @if ($rev->summary)
                                    <p class="mt-0.5 text-2xs italic text-brand-ink/75">{{ $rev->summary }}</p>
                                @endif
                            </div>
                            <div class="flex shrink-0 flex-col items-end gap-1">
                                @if ($webserverConfigCompareMode)
                                    <button type="button" wire:click="selectWebserverConfigRevisionForCompare('{{ $rev->id }}')" class="text-2xs font-medium text-brand-ink hover:underline">
                                        {{ $compareA ? __('Picked A') : ($compareB ? __('Picked B') : __('Select')) }}
                                    </button>
                                @else
                                    <button type="button" wire:click="showWebserverConfigRevisionDiff('{{ $rev->id }}')" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">{{ __('Diff') }}</button>
                                    <button type="button" wire:click="loadWebserverConfigRevision('{{ $rev->id }}')" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">{{ __('Load') }}</button>
                                    <button type="button" wire:click="rollbackWebserverConfigRevision('{{ $rev->id }}')" class="text-2xs font-semibold text-brand-forest hover:underline">{{ __('Rollback') }}</button>
                                @endif
                            </div>
                        </li>
                    @endforeach
                ...
                @if ($webserverConfigRevisions->count() >= $webserverConfigRevisionsLimit)
                    <div class="border-t border-brand-ink/10 px-3 py-2">
                        <button type="button" wire:click="showOlderWebserverConfigRevisions" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">
