<div class="{{ $card }}">
    ...
    <div class="px-3 py-2.5 sm:px-4">
        <pre class="max-h-[min(55vh,28rem)] overflow-auto whitespace-pre-wrap break-all rounded-lg bg-zinc-950 px-3 py-2.5 font-mono text-xs leading-relaxed text-zinc-100 [scrollbar-color:rgb(82_82_91/0.45)_transparent]">@if ($cron_daemon_log_body !== null){{ $cron_daemon_log_body }}@else{{ __('Click "Load cron log".') }}@endif</pre>
    </div>
</div>
    ...
    @else
        <div class="flex flex-col gap-2 border-b border-brand-ink/10 bg-brand-sand/10 px-3 py-2.5 sm:flex-row sm:items-end sm:justify-between sm:px-4">
            <div class="min-w-0 flex-1">
                <x-input-label for="log_scheduler_id" value="{{ __('Scheduler') }}" class="!text-xs" />
            ...
            @if ($logSelectedHeartbeat)
                <button
                    ...
                    @class([
                        'inline-flex h-7 shrink-0 items-center gap-1 rounded-md border px-2 text-xs font-semibold shadow-sm transition disabled:opacity-50',
        ...
        @if ($logTickOutputs->isEmpty())
            <div class="px-3 py-5 text-center sm:px-4">
                ...
                <p class="mt-0.5 text-xs text-brand-mist">{{ __('Failures record automatically; enable capture above to also keep successful output.') }}</p>
            </div>
        @else
                    ...
                    <li class="px-3 py-2.5 sm:px-4">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <div class="flex flex-wrap items-center gap-1.5">
                                <span @class([
                                    'inline-flex items-center rounded-full px-1.5 py-0.5 text-2xs font-semibold uppercase tracking-wide ring-1',
                                ...
                                @if ($tick->trigger === \App\Models\SchedulerTickOutput::TRIGGER_MANUAL)
                                    <span class="inline-flex items-center rounded-full bg-brand-sand/60 px-1.5 py-0.5 text-2xs font-semibold uppercase tracking-wide text-brand-moss">{{ __('Manual') }}</span>
                                @endif
                                @if ($tick->duration_ms !== null)
                                    <span class="text-xs text-brand-mist">{{ $tick->duration_ms }} ms</span>
                                @endif
                            </div>
                            <time class="text-xs text-brand-mist" datetime="{{ optional($tick->ran_at)->toIso8601String() }}" title="{{ optional($tick->ran_at)->toDayDateTimeString() }}">{{ optional($tick->ran_at)->diffForHumans() ?? '—' }}</time>
                        </div>
                        @if ($tick->stderr_excerpt)
                            <pre class="mt-1.5 max-h-48 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-zinc-950 px-2.5 py-2 font-mono text-xs leading-relaxed text-rose-200">{{ $tick->stderr_excerpt }}</pre>
                        @endif
                        @if ($tick->stdout_excerpt)
                            <pre class="mt-1.5 max-h-48 overflow-auto whitespace-pre-wrap break-all rounded-lg bg-zinc-950 px-2.5 py-2 font-mono text-xs leading-relaxed text-zinc-200">{{ $tick->stdout_excerpt }}</pre>
                        @endif
                    </li>
                @endforeach
            </ul>
