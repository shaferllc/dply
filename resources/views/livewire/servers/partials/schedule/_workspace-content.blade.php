...
@if ($contextSite)
    <div class="mb-4 flex items-center justify-between rounded-lg border border-brand-ink/15 bg-brand-sand/30 px-4 py-3 text-sm">
        ...
        <a href="{{ route('servers.schedule', $server) }}" wire:navigate class="text-xs font-semibold text-brand-ink underline">{{ __('View all schedulers on server') }}</a>
    </div>
@endif
    ...
    <section class="{{ $card }} overflow-hidden">
        <div class="flex items-start gap-3 border-b border-brand-ink/10 bg-brand-sand/20 px-6 py-5 sm:px-7">
            ...
            <div class="min-w-0">
                ...
                <h2 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Enable scheduler for a site') }}</h2>
                <p class="mt-1 max-w-2xl text-sm leading-relaxed text-brand-moss">{{ __('Creates a cron entry under the site\'s system user. Pre-flight validation + wrapper-managed entries land in a follow-on milestone.') }}</p>
            </div>
        </div>
