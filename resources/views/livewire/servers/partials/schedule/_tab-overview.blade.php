...
<section class="border-b border-brand-ink/10">
    ...
    @if ($attentionCards !== [])
        <ul class="divide-y divide-brand-ink/10">
            @foreach ($attentionCards as $cardData)
                ...
                @endphp
                <li class="flex flex-wrap items-start justify-between gap-2 px-3 py-2.5 sm:px-4">
                    <div class="min-w-0">
    ...
    @else
        <div class="flex flex-wrap items-center gap-2 px-3 py-3 sm:px-4">
            <p class="min-w-0 flex-1 text-xs text-brand-moss">{{ __('No schedulers need attention right now.') }}</p>
