            ...
            @foreach ($subscriptionsByChannel as $channelId => $subs)
                ...
                <li class="flex flex-wrap items-center gap-x-2 gap-y-1 px-4 py-2 sm:px-5" wire:key="health-notif-ch-{{ $channelId }}">
                    ...
                    <div class="ml-auto flex flex-wrap items-center gap-1.5">
                        @foreach ($subs as $sub)
                            <span class="inline-flex items-center gap-1 rounded-full bg-brand-sand/40 px-2 py-0.5 text-2xs font-semibold text-brand-ink ring-1 ring-inset ring-brand-ink/10" wire:key="health-notif-sub-{{ $sub->id }}">
