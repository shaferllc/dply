...
{{-- Nested inside the merged Health card — dense heads, same as the Security
     digest and Deploys notification tabs. The icon-badge + eyebrow + title +
     prose stack and the standalone cadence paragraph below it cost ~170px
     before the first channel row; the cadence is a clause in the head now. --}}
<div>
    ...
    @if ($subscriptionsByChannel->isEmpty())
        <p class="flex flex-wrap items-center gap-x-1.5 gap-y-1 border-b border-brand-ink/10 px-4 py-2.5 text-[11px] text-brand-moss sm:px-5">
            ...
            {{ __('No external channels are routed for health events yet — add one below to get an email or chat message when this server’s health degrades.') }}
        </p>
    @else
        <ul class="divide-y divide-brand-ink/10 border-b border-brand-ink/10">
            @foreach ($subscriptionsByChannel as $channelId => $subs)
                ...
                <li class="flex flex-wrap items-center gap-x-2 gap-y-1 px-4 py-2 sm:px-5" wire:key="health-notif-ch-{{ $channelId }}">
                    ...
                    <div class="ml-auto flex flex-wrap items-center gap-1.5">
                        @foreach ($subs as $sub)
                            <span class="inline-flex items-center gap-1 rounded-full bg-brand-sand/40 px-2 py-0.5 text-[10px] font-semibold text-brand-ink ring-1 ring-inset ring-brand-ink/10" wire:key="health-notif-sub-{{ $sub->id }}">
    ...
    <x-workspace-panel-head
        ...
        :note="__('Pick a channel, then the health events it should receive.')"
        class="border-b border-brand-ink/10"
    />

    <div class="px-4 py-3.5 sm:px-5">
        <form wire:submit="addHealthNotificationSubscription" class="space-y-3">
