...
{{-- Nested inside the merged Health card — same chrome as Overview / Deploys Notifications. --}}
<div>
    <div class="flex flex-col gap-3 border-b border-brand-ink/10 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-6">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                ...
                <h3 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Health alerts') }}</h3>
                <p class="mt-1 max-w-2xl text-sm leading-relaxed text-brand-moss">
                    {{ __('Route a channel (email, Slack, Discord, webhook…) to this server’s health cockpit — critical / warning alerts and recoveries.') }}
                </p>
            </div>
        </div>
        <a
    ...
    <p class="border-b border-brand-ink/10 px-5 py-3 text-xs leading-relaxed text-brand-moss sm:px-6">
        {{ __('The cockpit is evaluated on the background health cadence. Alerts only fire when overall posture worsens into warning / critical or recovers — not for every standing issue.') }}
    </p>
