{{--
    Lazy-load skeleton for Certificates. Mirrors the merged page (hide-hero +
    single card with identity, tabs, and inventory rows).
--}}
...
>
    <section class="dply-card min-w-0 overflow-hidden p-0" aria-busy="true" aria-live="polite">
        <span class="sr-only">{{ __('Loading certificates…') }}</span>
        ...
        <div class="border-b border-brand-ink/10 bg-brand-sand/20 px-5 py-5 sm:px-6" aria-hidden="true">
            <div class="flex flex-wrap items-start justify-between gap-4">
                ...
                <span class="inline-flex h-7 w-28 animate-pulse rounded-full bg-brand-ink/10"></span>
            </div>
        </div>

        <div class="flex flex-wrap gap-1.5 border-b border-brand-ink/10 px-4 py-2.5" aria-hidden="true">
            @foreach ([__('Certificates'), __('Notifications')] as $i => $label)
                <span @class([
        ...
        <div class="grid gap-3 px-5 py-5 sm:grid-cols-2 sm:px-6" aria-hidden="true">
            @foreach (range(1, 2) as $tile)
                <div class="h-32 animate-pulse rounded-2xl bg-brand-ink/10"></div>
            @endforeach
        </div>

        <ul class="divide-y divide-brand-ink/10 border-t border-brand-ink/10" aria-hidden="true">
