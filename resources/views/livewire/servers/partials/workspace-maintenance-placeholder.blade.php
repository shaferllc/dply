            ...
            <div class="flex min-w-0 items-start gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-1 max-w-2xl text-sm leading-relaxed text-brand-moss">
                        {{ __('Visitor maintenance window, site impact, and related downtime controls for this server.') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="border-b border-brand-ink/10" aria-hidden="true">
            ...
            <div class="grid gap-px bg-brand-ink/10 sm:grid-cols-3 xl:grid-cols-6">
                @foreach (range(1, 6) as $stat)
                    <div class="space-y-2 bg-white px-4 py-3.5">
                        <div class="h-2 w-16 animate-pulse rounded bg-brand-ink/10"></div>
                        <div class="h-6 w-10 animate-pulse rounded bg-brand-ink/10"></div>
                    </div>
                @endforeach
            </div>
            <div class="space-y-3 px-5 py-5 sm:px-6">
                @foreach (range(1, 3) as $row)
                    <div class="h-12 animate-pulse rounded-xl bg-brand-ink/10"></div>
                @endforeach
            </div>
        </div>
    </section>
