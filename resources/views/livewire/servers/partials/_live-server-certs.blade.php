...
<section class="{{ $liveCertsWrapperClass }}" wire:init="loadLiveCerts">
    ...
    @if ($liveCertsError)
        <p class="flex flex-wrap items-center gap-x-1.5 gap-y-1 border-b border-rose-200 bg-rose-50/70 px-4 py-2 text-xs text-rose-900 sm:px-5">
    ...
    @if ($liveCertsTimedOut)
        ...
        <div class="px-4 py-6 text-center text-xs text-brand-moss sm:px-5">
            ...
            @if (! empty($liveCertsProgress))
                ...
                <div class="mx-auto mt-3 max-h-40 max-w-xl overflow-y-auto rounded-md border border-brand-ink/10 bg-brand-ink/[0.03] px-3 py-2 text-left font-mono text-xs leading-relaxed text-brand-ink/70">
            ...
            <button
                ...
                class="mt-3 inline-flex h-7 items-center gap-1.5 rounded-lg border border-brand-ink/15 bg-white px-2.5 text-xs font-semibold text-brand-ink shadow-sm transition hover:bg-brand-sand/40 disabled:opacity-60"
            >
                <x-heroicon-m-arrow-path class="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
                {{ __('Retry scan') }}
            </button>
    ...
    @elseif (! $liveCertsLoaded)
        ...
        <div @if ($liveCertsScanning) wire:poll.{{ $this->liveCertsPollInterval() }}s="pollLiveCerts" @endif aria-busy="true" aria-live="polite">
            ...
            <p class="flex items-center gap-2 border-b border-brand-ink/10 px-4 py-2 text-xs text-brand-moss sm:px-5">
            ...
            <table class="min-w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-brand-ink/5 bg-white">
                    @foreach ($liveCerts as $cert)
                        ...
                        @endphp
                        <tr>
                            <td class="break-all px-3 py-2 font-mono text-xs text-brand-ink">{{ $cert['path'] }}</td>
                            ...
                            <td class="px-3 py-2">
                                <span class="inline-flex items-center rounded-full bg-brand-sand/40 px-2 py-0.5 text-2xs font-semibold uppercase tracking-wide text-brand-moss">{{ $cert['engine_hint'] ?? 'other' }}</span>
                            </td>
                            <td class="px-3 py-2 text-right">
                                @if ($cert['error'])
                                    <span class="text-xs text-rose-700" title="{{ $cert['error'] }}">—</span>
                                @else
                                    <span @class([
                                    ...
                                    @if (! empty($cert['not_after']))
                                        <p class="mt-0.5 text-2xs text-brand-mist tabular-nums">{{ $cert['not_after'] }}</p>
                                    @endif
                                @endif
                            </td>
                        </tr>
