...
{{-- Nested inside the merged Patches card — same chrome as Health Notifications. --}}
<div>
    <div class="flex flex-col gap-3 border-b border-brand-ink/10 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-6">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                ...
                <h3 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Patch alerts') }}</h3>
                <p class="mt-1 max-w-2xl text-sm leading-relaxed text-brand-moss">
                    {{ __('Route a notification channel (email, Slack, Discord, webhook…) to this server\'s patch events — updates applied, failures, reboots, and automatic-updates changes. Each row binds one channel to one event.') }}
                </p>
            </div>
        </div>
        <a
    ...
    <p class="border-b border-brand-ink/10 px-5 py-3 text-xs leading-relaxed text-brand-moss sm:px-6">
        {{ __('Owners and org admins already get an in-app notification (the bell) and inbox entry whenever updates are applied (or fail), the server reboots, or automatic updates are toggled — no setup needed. Add a channel below only to also send email / chat / webhook alerts. (This is separate from the server-level "Automatic updates" monitoring alert.)') }}
    </p>
        ...
        @if ($subscriptionsByChannel->isEmpty())
            <div class="rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 p-6 text-center">
                ...
                <p class="mt-3 text-sm text-brand-moss">
                    {{ __('No external channels are routed for patch events yet.') }}
                </p>
                <p class="mt-1 text-xs text-brand-mist">
                    {{ __('Add one below to get an email or chat message when updates are applied or a patch action fails.') }}
                </p>
            </div>
        @else
            <ul class="divide-y divide-brand-ink/10 rounded-xl border border-brand-ink/10 bg-white">
                @foreach ($subscriptionsByChannel as $channelId => $subs)
                    ...
                    <li class="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between" wire:key="patch-notif-ch-{{ $channelId }}">
                        <div class="min-w-0">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            @foreach ($subs as $sub)
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-brand-sand/40 px-2 py-1 text-[11px] font-medium text-brand-ink ring-1 ring-inset ring-brand-ink/10" wire:key="patch-notif-sub-{{ $sub->id }}">
                                    {{ $notifEventLabels[$sub->event_key] ?? $sub->event_key }}
                                    <button
                                        type="button"
                                        wire:click="removePatchNotificationSubscription(@js($sub->id))"
    ...
    <div class="border-t border-brand-ink/10 px-5 py-5 sm:px-6">
        ...
        <form wire:submit="addPatchNotificationSubscription" class="mt-4 space-y-4">
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    <x-input-label for="patch-notif-channel" value="{{ __('Channel') }}" />
                    <select
                        id="patch-notif-channel"
