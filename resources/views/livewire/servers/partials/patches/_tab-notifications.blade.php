...
<div class="{{ $card }}">
    <div class="flex flex-col gap-4 border-b border-brand-ink/10 px-6 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 sm:px-8">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                <h2 class="text-lg font-semibold text-brand-ink">{{ __('Patch alerts') }}</h2>
    ...
    <div class="mx-6 mt-5 flex items-start gap-2.5 rounded-xl border border-brand-ink/10 bg-brand-sand/15 px-4 py-3 text-sm leading-relaxed text-brand-moss sm:mx-8">
        ...
        <p>{{ __('Owners and org admins already get an in-app notification (the bell) and inbox entry whenever updates are applied (or fail), the server reboots, or automatic updates are toggled — no setup needed. Add a channel below only to also send email / chat / webhook alerts. (This is separate from the server-level "Automatic updates" monitoring alert.)') }}</p>
    </div>
