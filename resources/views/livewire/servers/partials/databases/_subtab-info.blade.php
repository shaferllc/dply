...
@if (\App\Support\Servers\DatabaseWorkspaceEngines::isMysqlFamily($engine))
    ...
    <div class="{{ $card }}">
        <x-workspace-panel-head
            ...
            :title="__('Live processlist')"
            :note="__('Runs SHOW PROCESSLIST against the engine over SSH. Output streams into the banner above.')"
            :class="$manageDbHasCreds ? '' : 'border-b border-brand-ink/10'"
        >
    ...
    <div id="db-manage-hints-{{ $engine }}" class="{{ $card }} scroll-mt-24">
        <x-workspace-panel-head
            dense
            icon="heroicon-o-link"
            :title="__('Database connection hints')"
            :note="__('Optional values for Dply features such as backups, and to authenticate the processlist action above.')"
            class="border-b border-brand-ink/10"
        />
        <form wire:submit="saveManageDbHints" class="max-w-xl space-y-4 px-4 py-3.5 sm:px-5">
            <div>
