...
<section class="space-y-6" aria-labelledby="manage-configuration-title">
    ...
    {{-- Clone server card. DigitalOcean droplet snapshot → new droplet, in the
         same region/size as the source. The dply-side row gets a draft entry
         with copied operator-intent meta (preset, runtime defaults, host_kind,
         DO opts); the heavy lifting runs in CloneServerOnDigitalOceanJob via
         the manage-action banner. --}}
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div class="max-w-2xl">
                    ...
                    <p class="mt-2 text-xs text-brand-mist">
                        {{ __('Copies operator intent (preset, runtime defaults, host kind, DigitalOcean options). Does NOT copy IP, SSH keys, sites, databases, or activity log — those belong to the new box.') }}
                    </p>
                </div>
                <button
                    type="button"
