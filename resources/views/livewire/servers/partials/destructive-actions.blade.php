        ...
        <ul class="divide-y divide-brand-ink/10">
            @foreach ($databases->sortBy('name') as $db)
                <li class="flex flex-wrap items-center gap-x-2 gap-y-2 px-4 py-3 sm:px-5">
                    ...
                    <span class="inline-flex items-center rounded-md bg-brand-sand/50 px-1.5 py-0.5 text-[11px] text-brand-ink/80 ring-1 ring-brand-ink/10">
                    ...
                    <div class="ml-auto flex shrink-0 flex-wrap gap-1.5">
                        <button
                            ...
                            class="inline-flex h-7 items-center whitespace-nowrap rounded-md border border-brand-ink/15 bg-white px-2 text-[11px] font-medium text-brand-ink shadow-sm transition hover:bg-brand-sand/40"
                        >
                            {{ __('Remove from Dply') }}
                        </button>
                        <button
                            ...
                            class="inline-flex h-7 items-center whitespace-nowrap rounded-md border border-red-200 bg-red-50 px-2 text-[11px] font-medium text-red-700 shadow-sm transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                            {{ __('Drop on server') }}
                        </button>
                    </div>
