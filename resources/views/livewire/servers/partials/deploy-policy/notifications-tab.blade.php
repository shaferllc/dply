...
{{-- Nested inside the merged Deploys card — same chrome as History / Coverage. --}}
<div>
    <div class="flex flex-col gap-3 border-b border-brand-ink/10 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-6">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                ...
                <h3 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Deploy window alerts') }}</h3>
                <p class="mt-1 max-w-2xl text-sm leading-relaxed text-brand-moss">
                    {{ __('Route a channel (email, Slack, Discord, webhook…) to deploy-window events — a blocked deploy, or enforcement toggled on / off.') }}
                </p>
            </div>
        </div>
        <a
    ...
    <p class="border-b border-brand-ink/10 px-5 py-3 text-xs leading-relaxed text-brand-moss sm:px-6">
        {{ __('Blocked deploys still appear on each site’s Deploys timeline. Channels below add email / chat / webhook alerts when a deny window holds one back.') }}
    </p>
