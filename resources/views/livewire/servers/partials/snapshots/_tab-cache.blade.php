<x-server-workspace-tab-panel id="snapshots-panel-cache" labelled-by="snapshots-tab-cache" panel-class="min-w-0">
    ...
    <section class="border-b border-brand-ink/10">
        <x-workspace-panel-head
            ...
            :title="__('Run a snapshot now')"
            :note="__('BGSAVE on the primary redis-family engine, copy the RDB file to your destination, and record the result below.')"
        ...
        <div class="px-4 py-3.5 sm:px-5">
            @if ($cacheServices->isEmpty())
                <div class="rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 px-4 py-6 text-center">
                    ...
                    <p class="mt-2.5 text-sm font-semibold text-brand-ink">{{ __('No cache service installed') }}</p>
            ...
            @elseif ($destinations->isEmpty())
                <button
                    ...
                    class="group block w-full rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 px-4 py-6 text-center transition hover:border-brand-sage/40 hover:bg-brand-sand/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-sage/40"
                >
                    <span class="mx-auto flex h-9 w-9 items-center justify-center rounded-xl bg-amber-50 text-amber-700 ring-1 ring-amber-200">
                        <x-heroicon-o-cloud class="h-4 w-4" aria-hidden="true" />
                    </span>
                    <p class="mt-2.5 text-sm font-semibold text-brand-ink">{{ __('No backup destination configured') }}</p>
                    <p class="mx-auto mt-1 max-w-md text-xs leading-relaxed text-brand-moss">
                        {{ __('Add an S3-style backup destination, then come back to capture your first snapshot.') }}
                    </p>
                    <span class="mt-2.5 inline-flex items-center gap-1 text-xs font-semibold text-brand-forest group-hover:underline">
    ...
    <section class="border-b border-brand-ink/10">
        <x-workspace-panel-head
            ...
            :title="__('Recurring snapshots')"
            :count="$schedules->isEmpty() ? null : (string) $schedules->count()"
            :note="__('Cron-driven captures controlled by the dply control plane. Each fires BGSAVE + upload at the configured cadence.')"
            class="border-b border-brand-ink/10"
        />
                        ...
                        <li class="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5">
                            ...
                            <div class="flex shrink-0 items-center gap-2">
                                <button
                                    ...
                                    class="inline-flex h-7 items-center gap-1 rounded-lg border border-brand-ink/15 bg-white px-2.5 text-xs font-medium text-brand-ink hover:bg-brand-sand/40"
                                >
                                    @if ($schedule->is_active)
                                ...
                                <button
                                    ...
                                    class="inline-flex h-7 items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2.5 text-xs font-medium text-red-700 hover:bg-red-100"
                                >
    ...
    <section class="border-b border-brand-ink/10">
        <x-workspace-panel-head
            ...
            :title="__('Recent snapshots')"
            :count="$snapshots->isEmpty() ? null : (string) $snapshots->count()"
            :note="__('50 most-recent runs. Failed snapshots keep their error trail for diagnosis.')"
            class="border-b border-brand-ink/10"
        />
        ...
        <div class="px-4 py-3.5 sm:px-5">
            @if ($snapshots->isEmpty())
                <div class="rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 px-4 py-6 text-center">
                    ...
                    <p class="mt-2.5 text-sm font-semibold text-brand-ink">{{ __('No snapshots yet') }}</p>
