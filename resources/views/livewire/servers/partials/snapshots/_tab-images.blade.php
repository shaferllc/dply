...
<x-server-workspace-tab-panel id="snapshots-panel-images" labelled-by="snapshots-tab-images" panel-class="min-w-0">
    ...
    <section class="border-b border-brand-ink/10">
        <x-workspace-panel-head
            ...
            :title="__('Capture a full-disk image')"
            :note="__('A complete, restorable image of this server taken through :provider. Stored on your cloud account and billed by them.', ['provider' => $server->provider?->label() ?? __('your provider')])"
        ...
        <div class="px-4 py-3.5 sm:px-5">
            @if (! $imagesSupported)
                <div class="rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 px-4 py-6 text-center">
                    ...
                    <p class="mt-2.5 text-sm font-semibold text-brand-ink">{{ __('Not available on :provider', ['provider' => $server->provider?->label() ?? __('this provider')]) }}</p>
    ...
    <section class="border-b border-brand-ink/10" @if ($imagesInFlight) wire:poll.10s @endif>
        <x-workspace-panel-head
            ...
            :title="__('Server images')"
            :count="$serverImages->isEmpty() ? null : (string) $serverImages->count()"
            :note="__('Images captured from this server. Deleting one removes it from your cloud account.')"
            class="border-b border-brand-ink/10"
        />
        ...
        <div class="px-4 py-3.5 sm:px-5">
            @if ($serverImages->isEmpty())
                <div class="rounded-xl border border-dashed border-brand-ink/15 bg-brand-sand/15 px-4 py-6 text-center">
                    ...
                    <p class="mt-2.5 text-sm font-semibold text-brand-ink">{{ __('No images yet') }}</p>
                    ...
                    <table class="min-w-full divide-y divide-brand-ink/10 text-xs">
                        <thead class="bg-brand-sand/40 text-left text-[10px] font-semibold uppercase tracking-wide text-brand-mist">
                            <tr>
                                ...
                                <th class="px-3 py-2">{{ __('Region') }}</th>
                        ...
                        <tbody class="divide-y divide-brand-ink/10 bg-white">
                            @foreach ($serverImages as $image)
                                ...
                                <tr wire:key="server-image-{{ $image->id }}">
                                    ...
                                    <td class="px-3 py-2 text-right font-mono tabular-nums text-brand-moss" @if ($imageCost !== null) title="{{ __('Estimated monthly storage on :provider — billed by them, not Dply.', ['provider' => $server->provider?->label() ?? __('your provider')]) }}" @endif>{{ $imageCost !== null ? $imageCost.'/mo' : '—' }}</td>
                                    <td class="px-3 py-2 text-brand-moss">{{ $image->region ?? '—' }}</td>
