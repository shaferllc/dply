{{--
    Lazy-load skeleton for Hygiene. Mirrors the merged page (hide-hero + single
...
>
    <section class="dply-card min-w-0 overflow-hidden p-0" aria-busy="true" aria-live="polite">
        <span class="sr-only">{{ __('Loading hygiene…') }}</span>
        ...
        <div class="flex flex-wrap gap-1.5 border-b border-brand-ink/10 px-4 py-2.5" aria-hidden="true">
            @foreach ([__('Overview'), __('Releases'), __('Logs & jobs'), __('Notifications')] as $i => $label)
                <span @class([
