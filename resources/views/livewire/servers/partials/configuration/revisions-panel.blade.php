        ...
        @if ($configDiffText !== null)
            <div class="mt-3 rounded-xl border border-brand-ink/10 bg-white p-3">
                <div class="flex items-center justify-between gap-2">
                    ...
                    <button type="button" wire:click="closeConfigRevisionDiff" class="text-xs font-medium text-brand-moss hover:text-brand-ink">
                        {{ __('Close diff') }}
                    </button>
                </div>
        ...
        @if ($configRevisions->isNotEmpty())
            <div class="mt-3 rounded-xl border border-brand-ink/10 bg-white">
                <div class="flex items-center justify-between border-b border-brand-ink/10 px-3 py-2">
                    ...
                    @if (! $isDeployer)
                        <button type="button" wire:click="toggleConfigCompareMode" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">
                        ...
                        <li class="flex items-start justify-between gap-3 px-3 py-2 {{ $isCurrent ? 'bg-emerald-50/40' : '' }}" wire:key="cfg-rev-{{ $rev->id }}">
                            ...
                            <div class="flex shrink-0 flex-col items-end gap-1">
                                @if ($configCompareMode && ! $isDeployer)
                                    <button type="button" wire:click="selectConfigRevisionForCompare('{{ $rev->id }}')" class="text-2xs font-medium text-brand-ink hover:underline">
                                        {{ $compareA ? __('Picked A') : ($compareB ? __('Picked B') : __('Select')) }}
                                    </button>
                                @else
                                    <button type="button" wire:click="showConfigRevisionDiff('{{ $rev->id }}')" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">{{ __('Diff') }}</button>
                                    @if (! $isDeployer)
                                        <button type="button" wire:click="loadConfigRevision('{{ $rev->id }}')" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">{{ __('Load') }}</button>
                                        <button type="button" wire:click="rollbackConfigRevision('{{ $rev->id }}')" class="text-2xs font-semibold text-brand-forest hover:underline">{{ __('Rollback') }}</button>
                                    @endif
                                @endif
                            </div>
                        </li>
                ...
                @if ($configRevisions->count() >= $configRevisionsLimit)
                    <div class="border-t border-brand-ink/10 px-3 py-2">
                        <button type="button" wire:click="showOlderConfigRevisions" class="text-2xs font-medium text-brand-moss hover:text-brand-ink">
