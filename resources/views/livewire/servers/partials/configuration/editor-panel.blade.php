        ...
        <div class="shrink-0 flex flex-wrap items-center justify-between gap-2">
            <div class="min-w-0">
                ...
                @elseif ($config_loaded_from_cache)
                    <p class="mt-1 inline-flex flex-wrap items-center gap-1.5 text-xs text-brand-moss">
                ...
                @else
                    <p
                        ...
                        x-cloak
                        class="mt-1 inline-flex flex-wrap items-center gap-1.5 text-xs text-brand-moss"
                    >
                        <span class="inline-flex items-center gap-1 rounded-md bg-sky-50 px-1.5 py-0.5 font-medium text-sky-800 ring-1 ring-sky-200">
                            <x-heroicon-o-bolt class="h-3 w-3" />
                            {{ __('Cached copy') }}
            ...
            <div class="flex flex-wrap gap-1.5">
                ...
                @else
                    <span class="inline-flex items-center rounded-md bg-brand-sand/50 px-2 py-1 text-2xs font-medium text-brand-moss ring-1 ring-brand-ink/10">
                        {{ __('Read-only') }}
                    </span>
                @endif
            </div>
            ...
            @if (! $isDeployer && ! empty($config_backups))
                <div class="mt-3 rounded-xl border border-brand-ink/10 bg-white">
                    <div class="flex items-center justify-between border-b border-brand-ink/10 px-3 py-2">
                        ...
                        <span class="text-2xs text-brand-mist">{{ __(':n kept — newest first', ['n' => count($config_backups)]) }}</span>
                    </div>
