    ...
    <div class="{{ $card ?? 'dply-card overflow-hidden' }} p-6 sm:p-8" x-data="{ tab: 'laravel' }">
        ...
        @if ($isMemcached)
            <div x-show="tab === 'laravel'" x-cloak>
                <pre class="mt-4 overflow-x-auto rounded-xl border border-brand-ink/10 bg-zinc-50 p-4 font-mono text-xs text-brand-ink"># Engine: {{ $engineLabel }} on 127.0.0.1:{{ $cacheService->port }}
