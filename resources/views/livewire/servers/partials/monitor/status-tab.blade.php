...
<div>
    <div class="flex flex-col gap-4 border-b border-brand-ink/10 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 sm:px-6">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                ...
                <h3 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Monitor status') }}</h3>
    ...
    @if ($routingSummary['server_routes'] === 0 && $opsReady)
        <div class="border-b border-brand-ink/10 bg-amber-50/50 px-5 py-4 sm:px-6">
    ...
    @if ($showMetricsPanels)
        ...
        <div class="border-b border-brand-ink/10 px-5 py-5 sm:px-6">
            <header class="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between">
                <div>
                    <h3 class="text-base font-semibold text-brand-ink">{{ __('Current usage') }}</h3>
                    @if ($latest)
                        <p class="mt-1 text-xs text-brand-mist">
            ...
            <div x-data="{ editing: @js($editingThresholds) }" x-init="$watch('editing', value => { if (!value) $wire.editingThresholds = false; })">
                <div class="flex flex-col gap-4 border-b border-brand-ink/10 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 sm:px-6">
                    <div class="flex min-w-0 items-start gap-3">
                        ...
                        <div class="min-w-0">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-brand-mist">{{ __('Thresholds') }}</p>
                            <h3 class="mt-0.5 text-base font-semibold text-brand-ink">{{ __('Alert thresholds') }}</h3>
