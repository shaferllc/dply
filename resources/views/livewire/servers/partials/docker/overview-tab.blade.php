...
<div class="min-w-0">
    <div class="border-b border-brand-ink/10">
        ...
        >
            <x-slot:actions>
                @if ($docker_present)
                    <span class="inline-flex h-6 shrink-0 items-center gap-1 whitespace-nowrap rounded-full bg-emerald-50 px-2 text-xs font-semibold text-emerald-800 ring-1 ring-emerald-200">
                        <span aria-hidden="true" class="inline-block h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                        {{ __('Installed') }}
                    </span>
                @else
                    <span class="inline-flex h-6 shrink-0 items-center gap-1 whitespace-nowrap rounded-full bg-white px-2 text-xs font-semibold text-brand-moss ring-1 ring-brand-ink/10">
                        {{ __('Not detected') }}
                    </span>
                @endif
                ...
                @unless ($docker_present)
                    @if ($installAction)
                        <button
                            ...
                            class="inline-flex h-6 shrink-0 items-center gap-1 whitespace-nowrap rounded-md bg-brand-ink px-2 text-xs font-semibold text-brand-cream shadow-sm transition-colors hover:bg-brand-forest"
                        >
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-brand-ink/10 bg-brand-cream/40 px-4 py-2.5 sm:px-5">
            <div class="min-w-0">
                ...
                @if (! ($canCreateDockerSite ?? false) && filled($siteCreateBlockedReason ?? ''))
                    <p class="mt-1.5 text-xs leading-relaxed text-amber-900">{{ $siteCreateBlockedReason }}</p>
                @endif
            </div>
            @if ($canCreateDockerSite ?? false)
                <a
                    ...
                    wire:navigate
                    class="inline-flex h-7 shrink-0 items-center gap-1.5 rounded-lg bg-brand-ink px-2.5 text-xs font-semibold text-brand-cream shadow-sm transition-colors hover:bg-brand-forest"
                >
                    {{ __('Create Docker site') }}
                    <x-heroicon-m-arrow-right class="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
                </a>
            @else
                <span class="inline-flex h-7 shrink-0 cursor-not-allowed items-center gap-1.5 rounded-lg bg-brand-mist/30 px-2.5 text-xs font-semibold text-brand-moss">
