{{--
  K8s create-new node-pool droplet size picker. Same anatomy as the Step 2
  _provider-size-picker (RAM/CPU/Disk lines, price on the right, recommended
  / too-small badges) but bound to form.do_kubernetes_new_node_size and
  without the recommendation engine (no role-based hints for K8s nodes yet).

  Required vars:
    - $sizes : list<array{value, label, price_monthly?, memory_mb?, vcpus?, disk_gb?}>
    - $form  : ServerCreateForm
--}}
@php
    $sizeCards = collect($sizes ?? [])->map(function (array $opt): array {
        $value = (string) ($opt['value'] ?? '');
        $memMb = (int) ($opt['memory_mb'] ?? 0);
        $vcpus = (int) ($opt['vcpus'] ?? 0);
        $diskGb = (int) ($opt['disk_gb'] ?? 0);
        $monthly = $opt['price_monthly'] ?? null;
        $ram = $memMb >= 1024 ? (int) round($memMb / 1024).'GB' : ($memMb > 0 ? $memMb.'MB' : '—');
        $cpu = $vcpus > 0 ? $vcpus.' vCPU' : '—';
        $disk = $diskGb > 0 ? $diskGb.'GB disk' : '—';
        $price = is_numeric($monthly) ? '$'.number_format((float) $monthly, ((float) $monthly) < 10 ? 2 : 0).'/mo' : null;
        // K8s system pods + workloads rarely fit in <2GB; flag below that.
        $tooSmall = $memMb > 0 && $memMb < 2048;
        ...
        return [
            ...
            'disk' => $disk,
            'price' => $price,
            'price_monthly' => is_numeric($monthly) ? (float) $monthly : null,
            'too_small' => $tooSmall,
        ];
    })->values();
    $selectedSizeCard = $sizeCards->firstWhere('value', $form->do_kubernetes_new_node_size);
    // "Recommended starter" = cheapest size that's not flagged too-small. Same
    // heuristic as the cost-preview's starter sample so the badge matches the
    // sidebar's sample estimate.
    $recommendedSizeCard = $sizeCards
        ->reject(fn (array $c): bool => $c['too_small'])
        ->sortBy('price_monthly')
        ->first() ?: $sizeCards->first();
@endphp

<div>
    <x-input-label for="form_do_kubernetes_new_node_size" :value="__('Node droplet size')" />
    <div x-data="{ open: false }" class="relative mt-1">
        @if ($sizeCards->isEmpty())
            <div class="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-5 text-sm text-slate-500">
                {{ __('Select an account first to load droplet sizes.') }}
            </div>
        @else
            <button
                id="form_do_kubernetes_new_node_size"
                type="button"
                ...
                <div class="max-h-96 space-y-2 overflow-y-auto overscroll-contain pr-1">
                    @foreach ($sizeCards as $sizeCard)
                        <button
                            type="button"
                            role="option"
                            wire:click="$set('form.do_kubernetes_new_node_size', '{{ $sizeCard['value'] }}')"
                            x-on:click="open = false"
                            aria-selected="{{ $form->do_kubernetes_new_node_size === $sizeCard['value'] ? 'true' : 'false' }}"
                            class="w-full rounded-xl border px-4 py-3 text-left transition focus:outline-none focus:ring-2 focus:ring-sky-500/30 {{ $form->do_kubernetes_new_node_size === $sizeCard['value'] ? 'border-sky-600 bg-sky-50 ring-1 ring-sky-600/20' : 'border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50' }}"
                        >
                            <div class="flex items-start justify-between gap-4">
    ...
    <x-input-error :messages="$errors->get('form.do_kubernetes_new_node_size')" class="mt-1" />
</div>
