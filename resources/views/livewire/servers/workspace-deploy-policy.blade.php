...
>
    ...
    <div @class(['flex flex-wrap items-center justify-between gap-3 rounded-2xl px-5 py-3.5 text-sm ring-1', $overallTone])>
        ...
        <div class="text-xs">
            ...
            @else
                {{ trans_choice(':count deny rule|:count deny rules', $summary['rule_count'] ?? 0, ['count' => $summary['rule_count'] ?? 0]) }}
                · {{ __('Timezone :tz', ['tz' => $policyTz]) }}
            @endif
        </div>
    </div>

    <x-server-workspace-tablist :aria-label="__('Deploy window sections')">
        <x-server-workspace-tab id="dp-tab-overview" icon="heroicon-o-calendar-days" :active="$policy_tab === 'overview'" wire:click="setPolicyTab('overview')">
            {{ __('Overview') }}
        </x-server-workspace-tab>
        <x-server-workspace-tab id="dp-tab-schedule" icon="heroicon-o-adjustments-horizontal" :active="$policy_tab === 'schedule'" wire:click="setPolicyTab('schedule')">
            {{ __('Schedule') }}
        </x-server-workspace-tab>
        <x-server-workspace-tab id="dp-tab-coverage" icon="heroicon-o-globe-alt" :active="$policy_tab === 'coverage'" wire:click="setPolicyTab('coverage')">
            {{ __('Coverage') }}
            @if (($summary['total_sites'] ?? 0) > 0)
                <span class="inline-flex shrink-0 items-center rounded-full bg-brand-sand/80 px-1.5 py-0.5 text-[10px] font-semibold leading-none tabular-nums text-brand-moss">{{ $summary['total_sites'] }}</span>
            @endif
        </x-server-workspace-tab>
        <x-server-workspace-tab id="dp-tab-activity" icon="heroicon-o-no-symbol" :active="$policy_tab === 'activity'" wire:click="setPolicyTab('activity')">
            {{ __('Activity') }}
            @if (($summary['skipped_deploys_7d'] ?? 0) > 0)
                <span class="inline-flex shrink-0 items-center rounded-full bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold leading-none tabular-nums text-amber-900">{{ $summary['skipped_deploys_7d'] }}</span>
            @endif
        </x-server-workspace-tab>
        <x-server-workspace-tab id="dp-tab-notifications" icon="heroicon-o-bell" :active="$policy_tab === 'notifications'" wire:click="setPolicyTab('notifications')">
            {{ __('Notifications') }}
        </x-server-workspace-tab>
    </x-server-workspace-tablist>

    {{-- Skeleton placeholder shown while the incoming tab loads. --}}
    <div wire:loading.block wire:target="setPolicyTab">
        @include('livewire.servers.partials._skeleton-cards')
    </div>

    <div class="space-y-6" wire:loading.remove wire:target="setPolicyTab">

        {{-- ─── OVERVIEW ──────────────────────────────────────────────────── --}}
        @if ($policy_tab === 'overview')
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="mt-1 text-sm text-brand-moss">
                                @if ($overall === 'disabled')
                                    {{ __('Enable the policy on the Schedule tab to enforce deny windows for every site on this server.') }}
            ...
            <section class="dply-card overflow-hidden">
                <div class="flex items-start justify-between gap-3 border-b border-brand-ink/10 bg-brand-sand/20 px-6 py-5 sm:px-7">
                    ...
                    <button type="button" wire:click="setPolicyTab('schedule')" class="shrink-0 rounded-lg border border-brand-ink/15 bg-white px-3 py-1.5 text-xs font-semibold text-brand-ink shadow-sm hover:bg-brand-sand/40">
                        {{ __('Edit') }}
                    </button>
                </div>
                ...
                @if ($ruleRows === [])
                    <div class="px-6 py-10 text-center text-sm text-brand-moss sm:px-7">
                        {{ __('No deny rules yet — add rules on the Schedule tab or apply the weekend freeze preset.') }}
                    </div>
                @else
        ...
        {{-- ─── ACTIVITY (recent skips) ───────────────────────────────────── --}}
        @if ($policy_tab === 'activity')
            <section class="dply-card overflow-hidden">
                <div class="flex items-start gap-3 border-b border-brand-ink/10 bg-brand-sand/20 px-6 py-5 sm:px-7">
                    <x-icon-badge>
