...
<li class="py-3 first:pt-0 last:pb-0">
    <details>
        <summary @class([
            ...
            @if ($invocation->cold)
                <span class="inline-flex items-center rounded-md bg-brand-gold/15 px-1.5 py-0.5 text-2xs font-semibold text-brand-ink/70">{{ __('cold') }}</span>
            @endif
            ...
            @if ($invocation->activation_id)
                <span class="ml-auto font-mono text-xs text-brand-moss/50">{{ $invocation->activation_id }}</span>
            @elseif (! empty($invocation->context['ip']) || ! empty($invocation->context['country']))
                <span class="ml-auto font-mono text-xs text-brand-moss/50">{{ trim(($invocation->context['country'] ?? '').' '.($invocation->context['ip'] ?? '')) }}</span>
            @endif
        </summary>
            ...
            <dl class="mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5 rounded-lg bg-brand-sand/30 p-3 sm:grid-cols-3">
                @foreach ($contextPairs as $label => $value)
                    <div class="min-w-0">
                        ...
                        <dd class="truncate font-mono text-xs text-brand-ink" title="{{ $value }}">{{ $value }}</dd>
                    </div>
                @endforeach
            </dl>
        @endif
