...
<div class="border-t border-brand-ink/10">
    <x-workspace-panel-head
        ...
        :title="__('Network')"
        :note="__('The private network this app\'s database and servers share. New clusters are created inside it.')"
    >
        @if ($attached)
            <x-slot:actions>
        ...
        @else
            ...
            @if ($stale)
                <div class="rounded-xl border border-brand-gold/30 bg-brand-gold/10 px-4 py-3 text-sm text-brand-ink">
                    {{ __('This app\'s database was created before the network was attached. DigitalOcean cannot move an existing cluster between networks — only clusters provisioned from now on will join it.') }}
                </div>
            @endif
                ...
                <div>
                    ...
                    @if ($members->isEmpty())
                        <p class="mt-1 text-sm text-brand-moss">
                            {{ __('None yet. A server on this network can reach the app\'s database privately — useful for running migrations or opening a shell against it.') }}
                        </p>
                    @else
