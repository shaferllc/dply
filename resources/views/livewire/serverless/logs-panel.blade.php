...
<div class="dply-card overflow-hidden">
    ...
    @elseif ($tab === 'runtime')
        ...
        {{-- The stream above is a fixed slice of recent invocations and the
             pruner drops organic rows after 7 days, so it can neither be
             searched nor relied on for history. The drain is the way out of
             both: the same lines land in app_logs, searchable and kept for 30
             days. Offered here rather than in Settings because this is where
             the limitation is actually felt. --}}
        @if ($drainAvailable)
            <div class="mt-3 rounded-lg border border-brand-ink/10 bg-brand-sand/30 px-3 py-2.5">
                @if ($drainEnabled)
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <p class="text-xs font-semibold text-brand-ink">{{ __('Searchable log history is on') }}</p>
                        <button type="button" wire:click="disableLogDrain" wire:loading.attr="disabled"
                                class="text-2xs font-semibold text-brand-moss hover:text-rose-600 hover:underline">
                            {{ __('Turn off') }}
                        </button>
                    </div>
                    <p class="mt-0.5 text-2xs text-brand-moss">
                        {{ __('Shipping to dply, searchable and kept for 30 days. Applies from the next deploy onward.') }}
                    </p>
                    <div class="mt-2.5">
                        <livewire:sites.site-app-logs :site="$site" :embedded="true" wire:key="serverless-app-logs-{{ $site->id }}" />
                    </div>
                @else
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <div class="min-w-0">
                            <p class="text-xs font-semibold text-brand-ink">{{ __('Want searchable history?') }}</p>
                            <p class="mt-0.5 text-2xs text-brand-moss">
                                {{ __('Ship this function\'s log to dply for search, level filtering, and 30-day retention. Costs a reconnect per request — a function has no resident process to hold the socket open.') }}
                            </p>
                        </div>
                        <button type="button" wire:click="enableLogDrain" wire:loading.attr="disabled" wire:target="enableLogDrain"
                            ...
                            <span wire:loading.remove wire:target="enableLogDrain">{{ __('Turn on') }}</span>
                            <span wire:loading wire:target="enableLogDrain">{{ __('Attaching…') }}</span>
                        </button>
                    </div>
                @endif
            </div>
