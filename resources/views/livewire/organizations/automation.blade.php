@php
    ...
    $sectionHeader = function (string $eyebrow, string $title, string $description, string $icon) {
        return [
            'eyebrow' => $eyebrow,
            'title' => $title,
...
<div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
            ...
            <x-slot:stats>
                <dl class="grid grid-cols-3 gap-3" aria-label="{{ __('Automation at a glance') }}">
                    ...
                    >
                        <p class="mt-2 flex items-center gap-1.5">
                            @if ($deploy_email_notifications_enabled)
                                ...
                                <span class="text-sm font-semibold text-brand-forest">{{ __('On') }}</span>
                            @else
            ...
            {{-- Notifications: deploy emails + credential emails. --}}
            @php $h = $sectionHeader(__('Notifications'), __('Email defaults'), __('What dply emails about for sites and servers in this organization. Notification routing for channels lives on a separate page.'), 'heroicon-o-bell'); @endphp
            <section class="border-b border-brand-ink/10" id="notifications">
                ...
                <div class="divide-y divide-brand-ink/10">
                    <label class="flex cursor-pointer items-start gap-3 px-5 py-4 transition-colors hover:bg-brand-sand/15 sm:px-6">
            ...
            @php $h = $sectionHeader(__('Cloud alerts'), __('Alert destinations'), __('Where dply sends deploy-failed, restart, CPU, and memory alerts for Cloud apps. Org owners are always notified on their login emails — these fields add extra recipients.'), 'heroicon-o-exclamation-triangle'); @endphp
            <section class="border-b border-brand-ink/10" id="alerts">
                ...
                <form wire:submit="saveAlertDestinations" class="space-y-5 px-5 py-5 sm:px-6">
                    <div>
            ...
            @php $h = $sectionHeader(__('Data residency'), __('Edge data region'), __('Preferred Cloudflare R2 region for buckets created on behalf of this organization. Existing buckets stay where they are — the setting only applies to future Edge bootstraps.'), 'heroicon-o-globe-europe-africa'); @endphp
            ...
            {{-- API tokens --}}
            @php $h = $sectionHeader(__('Automation'), __('API tokens'), __('Scoped tokens for CI/CD and automation. The secret is shown once after creation — copy it into your secrets store immediately.'), 'heroicon-o-key'); @endphp
            <section class="border-b border-brand-ink/10" id="api-tokens">
                ...
                <form wire:submit="createApiToken" class="space-y-5 border-b border-brand-ink/10 px-5 py-5 sm:px-6">
                ...
                <div class="border-b border-brand-ink/10 bg-brand-sand/35 px-5 py-2.5 sm:px-6">
            ...
            @php $h = $sectionHeader(__('Outbound'), __('Webhook destinations'), __('Push deploy and monitoring events to Slack, Discord, or Microsoft Teams. Scope to one site or fire for every site in this organization.'), 'heroicon-o-link'); @endphp
            <section class="border-b border-brand-ink/10 last:border-b-0" id="webhooks">
                <div class="flex items-start gap-3 border-b border-brand-ink/10 bg-brand-sand/20 px-5 py-4 sm:px-6">
                    <x-icon-badge>
                        <x-dynamic-component :component="$h['icon']" class="h-5 w-5" aria-hidden="true" />
                    </x-icon-badge>
                ...
                <form wire:submit="saveWebhookDestination" class="space-y-4 border-b border-brand-ink/10 px-5 py-5 sm:px-6">
                ...
                @else
                    <ul class="divide-y divide-brand-ink/10">
                        @foreach ($organization->notificationWebhookDestinations as $hook)
                            <li wire:key="webhook-{{ $hook->id }}" class="flex items-center justify-between gap-4 px-5 py-3.5 transition-colors hover:bg-brand-sand/15 sm:px-6">
                                <div class="min-w-0 flex-1">
