<div>
    ...
    @if (!empty($oauthProviders))
        ...
        <p class="mb-5 text-center text-sm text-stone-500">Or sign in with email</p>
    @endif
    ...
    <form wire:submit="submit" class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox" wire:model="remember" class="rounded border-stone-300 text-stone-600 shadow-sm focus:ring-stone-500">
                <span class="ms-2 text-sm text-stone-600">{{ __('Remember me') }}</span>
            </label>
            <a class="text-sm text-stone-600 hover:text-stone-900 focus:outline-none focus:ring-2 focus:ring-stone-500 focus:ring-offset-2 rounded" href="{{ route('password.request') }}">
                {{ __('Forgot your password?') }}
            </a>
        </div>
        <div class="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-end gap-3 pt-1">
            <a href="{{ route('register') }}" class="text-sm text-stone-600 hover:text-stone-900 text-center sm:text-left">Create an account</a>
