<section class="overflow-hidden rounded-2xl border border-brand-ink/10 bg-white shadow-sm">
    <div class="flex flex-wrap items-start justify-between gap-3 border-b border-brand-ink/10 bg-brand-cream/40 px-6 py-4">
        <div class="min-w-0">
            <h3 class="text-base font-semibold text-brand-ink">{{ __('WordPress core') }}</h3>
            <p class="mt-0.5 text-sm text-brand-moss">{{ __('Installed version from `wp core version` with an availability check against WordPress.org.') }}</p>
        </div>
        @if ($coreLoaded)
            <button type="button" wire:click="loadCore" wire:loading.attr="disabled" wire:target="loadCore" class="inline-flex items-center gap-1.5 rounded-md border border-brand-ink/15 bg-white px-2.5 py-1.5 text-xs font-medium text-brand-ink hover:bg-brand-sand/40 disabled:opacity-50">
