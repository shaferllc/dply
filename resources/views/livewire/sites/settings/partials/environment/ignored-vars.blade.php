    ...
    @if ($canIgnoreEnv && $ignoredEnvKeys !== [])
        <div class="{{ $card }}">
            <div class="flex flex-wrap items-start justify-between gap-3 bg-brand-sand/20 px-5 py-4">
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-mist">{{ __('Ignored variables') }}</p>
                    ...
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        @foreach ($ignoredEnvKeys as $ik)
                            <span class="inline-flex items-center gap-1 rounded-full bg-white px-2 py-0.5 font-mono text-xs font-semibold text-brand-moss ring-1 ring-inset ring-brand-ink/10">
                                {{ $ik }}
