...
<section class="rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-sm sm:p-8 space-y-6">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-brand-moss">{{ __('What this site runs and how. Language and version live here; per-language tuning lives in the runtime sub-tab below.') }}</p>
        </div>
        @if ($runtimeSubTab !== null)
            <a href="{{ route('sites.show', ['server' => $server, 'site' => $site, 'section' => $runtimeSubTab]) }}" wire:navigate class="inline-flex shrink-0 items-center gap-2 rounded-xl border border-brand-ink/15 bg-white px-3 py-2 text-sm font-medium text-brand-ink hover:bg-brand-sand/50">
    ...
    @if ($showAppPortEditor)
        <form wire:submit="saveRuntimePreferences" class="rounded-2xl border border-brand-ink/10 bg-brand-sand/30 p-4 sm:flex sm:items-end sm:gap-4">
            <div class="flex-1">
...
<section class="mt-6 rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-sm sm:p-8 space-y-4">
    <div>
        <h2 class="text-lg font-semibold text-brand-ink">{{ __('Repository detection') }}</h2>
        <p class="mt-1 text-sm text-brand-moss">{{ __('What Dply inferred from your repository. Detection runs on deploy and container inspect.') }}</p>
    </div>
...
<section class="mt-6 rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-sm sm:p-8 space-y-4">
    <div class="flex flex-wrap items-baseline justify-between gap-2">
        <div>
            <h2 class="text-lg font-semibold text-brand-ink">{{ __('Site processes') }}</h2>
            <p class="mt-1 text-sm text-brand-moss">{{ __('The web row drives the NGINX upstream. Workers and schedulers run as separate systemd units (dply-site-:id-:name.service).', ['id' => $site->id, 'name' => '<name>']) }}</p>
        </div>
        @if ($site->processes->isNotEmpty())
            <p class="text-xs text-brand-moss">{{ trans_choice('{1} 1 process|[2,*] :count processes', $site->processes->count(), ['count' => $site->processes->count()]) }}</p>
        @endif
    </div>
    ...
    @else
        <div class="rounded-xl border border-brand-ink/10 bg-brand-sand/30 p-4 text-sm text-brand-moss">
...
@if ($site->usesDockerRuntime())
    @if ($dockerContainers->isNotEmpty() || $runtimePublication !== [])
        <section class="mt-6 rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-sm sm:p-8 space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 class="text-lg font-semibold text-brand-ink">{{ __('Docker discovery') }}</h2>
    ...
    @if ($site->usesLocalDockerHostRuntime())
        <section class="mt-6 rounded-2xl border border-brand-ink/10 bg-white p-6 shadow-sm sm:p-8 space-y-5">
            <div>
                <h2 class="text-lg font-semibold text-brand-ink">{{ __('Container lifecycle') }}</h2>
                <p class="mt-1 text-sm text-brand-moss">{{ __('Lifecycle and inspection for the local container runtime behind this app. Output and historical operations live on the Logs tab.') }}</p>
            </div>
...
<div class="mt-6 rounded-2xl border border-brand-ink/10 bg-brand-sand/30 p-4">
