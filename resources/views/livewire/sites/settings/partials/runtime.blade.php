            ...
            @if ($showPhpStackDetails)
                <div class="space-y-4 border-t border-brand-ink/10 pt-6">
                    ...
                    @if ($site->shouldShowOctaneRuntimeUi())
                        <p class="text-sm text-brand-moss">{{ __('Shown because inspection found a Laravel app with `laravel/octane` in composer.json. Set the Octane port to match Nginx/Caddy/Apache (managed config proxies to 127.0.0.1). Install your Octane server runtime on the host, then add a Supervisor program on the server’s Daemons tab using the Octane preset—edit `--port` and `--server` to match the values below.') }}</p>
                    @endif
                    @if ($site->shouldShowPhpOctaneRolloutSettings() && $site->shouldShowOctaneRuntimeUi())
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            <div>
                                <x-input-label for="runtime_octane_port" :value="__('Octane port')" />
                                <x-text-input id="runtime_octane_port" wire:model="octane_port" class="mt-1 block w-full font-mono text-sm" placeholder="8000" />
                                <x-input-error :messages="$errors->get('octane_port')" class="mt-1" />
                            </div>
                            <div>
                                <x-input-label for="runtime_octane_server" :value="__('Octane application server')" />
                                <select id="runtime_octane_server" wire:model="octane_server" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm text-sm">
                                    @foreach (\App\Models\Site::OCTANE_SERVERS as $octaneServer)
                                        <option value="{{ $octaneServer }}">{{ str($octaneServer)->replace('_', ' ')->title() }}</option>
                                    @endforeach
                                </select>
                                <p class="mt-1 text-xs text-brand-moss">{{ __('Used for the Supervisor preset command (`php artisan octane:start --server=…`). Also available under Deploy → Rollout and web server.') }}</p>
