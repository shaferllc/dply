...
@if ($site->isScaffoldInstalling() && ! ($generalTabChooseAppOnly ?? false))
    @php $scaffoldDidFail = $site->status === \App\Models\Site::STATUS_SCAFFOLD_FAILED; @endphp
    {{-- Flush strip, not a floating card: this renders INSIDE the merged
         dply-card, so it follows the same hairline rhythm as the fact rows
         below it (see _console-action-banner's $embeddedBanner note). A
         rounded, bordered block here reads as a card nested in a card. --}}
    <div class="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-brand-ink/10 {{ $scaffoldDidFail ? 'bg-rose-50/40' : 'bg-brand-sand/[0.18]' }} px-3 py-2.5 sm:px-4">
        <div class="flex min-w-0 items-start gap-2">
            <x-dynamic-component
                :component="$scaffoldDidFail ? 'heroicon-o-exclamation-triangle' : 'heroicon-o-arrow-path'"
                class="mt-px h-4 w-4 shrink-0 {{ $scaffoldDidFail ? 'text-rose-600' : 'text-brand-sage' }}"
                aria-hidden="true"
            />
            <div class="min-w-0">
                <p class="text-xs font-semibold text-brand-ink">
                    {{ $scaffoldDidFail ? __('App install failed') : __('App install running') }}
                </p>
                <p class="mt-0.5 text-2xs leading-relaxed text-brand-moss">
                    {{ $scaffoldDidFail
                        ? __('See which step failed, retry it, or start over with a different app.')
                        : __('Follow each step as it runs.') }}
                </p>
            </div>
        </div>
