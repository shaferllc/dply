            ...
            <div class="flex flex-col gap-2.5 px-3 py-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-4 sm:px-4">
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-1.5">
                        ...
                        @if ($site->isSuspended())
                            <span class="rounded-full bg-amber-100 px-1.5 py-0.5 text-2xs font-semibold uppercase tracking-wide text-amber-900 ring-1 ring-inset ring-amber-200">{{ __('Suspended') }}</span>
                        @endif
                    </div>
                ...
                @can('update', $site)
                    @if ($site->isSuspended())
                        <button type="button" wire:click="resumeSite" wire:loading.attr="disabled" wire:target="resumeSite" class="inline-flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-amber-300 bg-white px-2.5 py-1 text-xs font-semibold text-amber-900 shadow-sm transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60">
                    ...
                    @else
                        <button type="button" wire:click="confirmSuspendSite" wire:loading.attr="disabled" wire:target="confirmSuspendSite" class="inline-flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-amber-400 bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-900 shadow-sm transition hover:bg-amber-200 disabled:cursor-not-allowed disabled:opacity-60">
    ...
    @can('clone', $site)
        <section class="border-b border-brand-ink/10">
            <div class="flex flex-col gap-2.5 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-4">
                ...
                <a href="{{ route('sites.clone', [$server, $site]) }}" wire:navigate class="inline-flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-brand-ink/15 bg-white px-2.5 py-1 text-xs font-semibold text-brand-ink shadow-sm transition hover:bg-brand-sand/40 sm:self-center">
        ...
        @if ($server->isVmHost())
            <section class="border-b border-brand-ink/10">
                <div class="flex flex-col gap-2.5 px-3 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-4">
                    ...
                    <a href="{{ route('sites.promote', [$server, $site]) }}" wire:navigate class="inline-flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-brand-sage/30 bg-white px-2.5 py-1 text-xs font-semibold text-brand-forest shadow-sm transition hover:bg-brand-sage/10 sm:self-center">
        ...
        <section class="border-b border-rose-200 last:border-b-0">
            <div @class([
                ...
                @unless ($site->scheduled_deletion_at)
                    <button type="button" wire:click="openRemoveSiteModal" class="inline-flex shrink-0 items-center gap-1.5 self-start rounded-lg border border-rose-300 bg-white px-2.5 py-1 text-xs font-semibold text-rose-800 shadow-sm transition hover:bg-rose-100 sm:self-center">
            ...
            @if ($site->scheduled_deletion_at)
                <div class="px-3 py-2 sm:px-4">
                    <div class="flex flex-wrap items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-2.5 py-1.5 text-xs text-amber-900">
