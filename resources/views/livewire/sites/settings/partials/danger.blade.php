            ...
            <div class="rounded-xl border border-rose-200 bg-rose-50/60 p-4 sm:p-5">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-1 text-sm leading-relaxed text-brand-moss">{{ __('Removes the site from Dply. A background job removes the Nginx vhost, optional releases/repo/cert, supervisor rows tied to this site, the deploy SSH key, and re-syncs the server crontab.') }}</p>
                        <button type="button" wire:click="openConfirmActionModal('deleteSite', [], @js(__('Delete site')), @js(__('Delete this site from Dply? A background job removes Nginx vhost, optional releases/repo/cert, supervisor rows tied to this site, deploy SSH key, and re-syncs server crontab.')), @js(__('Delete site')), true)" class="mt-3 inline-flex items-center gap-1.5 rounded-lg border border-rose-300 bg-rose-50 px-3 py-1.5 text-xs font-semibold text-rose-800 shadow-sm transition hover:bg-rose-100">
