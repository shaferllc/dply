...
@if ($canLinkSecrets)
    <section class="{{ $secretsCard }}" x-data="{ open: @js(count($linkedSecretRows) <= 6) }">
        <div class="border-b border-brand-ink/10 bg-brand-sand/20 px-5 py-3.5 sm:px-6">
            ...
            <p class="mt-1 text-xs leading-relaxed text-brand-moss">
                {{ __('Org vault keys injected on the next deploy. They cannot be read back.') }}
            </p>
        </div>

        <div x-show="open" x-cloak>
            @if ($linkedSecretRows === [])
                <p class="px-5 py-5 text-sm text-brand-moss sm:px-6">{{ __('No secrets linked. Paste a key here or pick one from the org vault — it injects on the next deploy and cannot be read back.') }}</p>
            @else
                    ...
                    >
                        @if ($secretGroups->count() > 1)
                            <button
                                ...
                                class="flex w-full items-center gap-2 bg-brand-sand/15 px-5 py-2.5 text-left sm:px-6"
                            >
                                ...
                                <span class="text-xs font-semibold tabular-nums text-brand-mist">{{ $rows->count() }}</span>
                            </button>
                        @endif
                        <ul class="divide-y divide-brand-ink/8" @if ($secretGroups->count() > 1) x-show="expanded" x-cloak @endif>
                            @foreach ($rows as $row)
                                <li class="px-5 py-2.5 transition-colors hover:bg-brand-sand/15 sm:px-6" wire:key="linked-secret-{{ $row['id'] }}">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @can('update', $site)
                                            <div class="flex shrink-0 items-center gap-1">
                                                <button
                                                    type="button"
                                                    class="inline-flex shrink-0 items-center justify-center rounded-lg border border-brand-ink/15 bg-white p-2 text-brand-ink shadow-sm hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700"
                                                    title="{{ __('Unlink :key', ['key' => $row['key']]) }}"
                                                    aria-label="{{ __('Unlink :key', ['key' => $row['key']]) }}"
