...
<li class="flex gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
    <span class="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-2xs font-bold {{ $glyphClasses }}">{{ $glyph }}</span>
    <div class="min-w-0 flex-1">
        ...
        @if ($stepOutput !== '')
            <div x-data="{ open: @js($shouldRenderOutput) }" class="mt-2">
                ...
                <pre x-show="open" x-cloak class="mt-1 max-h-96 overflow-auto rounded bg-slate-900 p-3 font-mono text-xs text-slate-100">{{ $stepOutput }}</pre>
            </div>
        @endif
    </div>
</li>
