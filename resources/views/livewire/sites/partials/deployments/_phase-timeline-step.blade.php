...
<li>
    <div class="flex items-center gap-2 text-xs">
        <span class="inline-flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-full text-3xs font-bold {{ $step['glyph_classes'] }}">
            @if ($stepRunning)
        ...
        @if ($stepRunning)
            <span class="rounded bg-amber-100 px-1.5 py-0.5 text-3xs font-semibold uppercase tracking-[0.1em] text-amber-800 ring-1 ring-inset ring-amber-200/70">{{ __('running') }}</span>
        @elseif ($stepPending)
            <span class="rounded bg-brand-sand/40 px-1.5 py-0.5 text-3xs font-semibold uppercase tracking-[0.1em] text-brand-mist">{{ __('queued') }}</span>
        @elseif ($step['skipped'])
            <span class="rounded bg-amber-50 px-1.5 py-0.5 text-3xs font-semibold uppercase tracking-[0.1em] text-amber-800 ring-1 ring-inset ring-amber-200/70">{{ __('skipped') }}</span>
