...
<section id="pipeline-step-catalog" class="scroll-mt-24 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        ...
                                        @if (filled($entry['requires_label'] ?? null))
                                            <span class="inline-flex rounded-full bg-brand-sand px-1.5 py-0.5 text-2xs font-semibold text-brand-moss ring-1 ring-brand-ink/10">
                                                {{ $entry['requires_label'] }}
                                            </span>
                                        @endif
                                        @unless ($visible)
                                            <span class="inline-flex rounded-full bg-amber-100 px-1.5 py-0.5 text-2xs font-semibold text-amber-900">
                                                {{ __('Not in palette') }}
                                            </span>
                                        @endunless
                                    </div>
                                    @if (filled($entry['command_preview'] ?? null))
                                        <p class="mt-0.5 font-mono text-xs text-brand-moss">{{ $entry['command_preview'] }}</p>
                                    @endif
                                </div>
                                <button
                                    type="button"
    ...
    <details class="group border-b border-brand-ink/10">
        <summary class="flex cursor-pointer list-none items-center gap-2 px-5 py-2.5 marker:content-none sm:px-6">
            ...
            <span class="min-w-0">
                ...
                <span class="mt-0.5 block text-xs text-brand-moss">{{ __('Raw types available in the step editor.') }}</span>
            </span>
        </summary>
        <div class="border-t border-brand-ink/10 px-5 py-3 sm:px-6">
            <p class="text-xs text-brand-moss">{{ __('Use “Add step” on Pipeline for npm scripts or commands not listed as shortcuts above.') }}</p>
                    ...
                    <tbody class="divide-y divide-brand-ink/8">
                        @foreach ($stepTypeReference as $row)
                            <tr wire:key="step-type-ref-{{ $row['type'] }}">
                                ...
                                <td class="py-1.5 font-mono text-xs text-brand-moss">
    ...
    <div class="border-b border-brand-ink/10">
        ...
        <div class="px-5 py-3 sm:px-6">
            <p class="text-2xs font-semibold uppercase tracking-[0.12em] text-brand-mist">{{ __('Types') }}</p>
            <ul class="mt-1.5 flex flex-wrap gap-1.5">
                @foreach ($hookCatalog['types'] ?? [] as $hookType)
                    <li class="inline-flex items-center gap-1 rounded-full border border-amber-200/80 bg-amber-50/80 px-2 py-1 text-xs font-semibold text-amber-950">
            ...
            @if (($hookCatalog['presets'] ?? []) !== [])
                <p class="mt-3 text-2xs font-semibold uppercase tracking-[0.12em] text-brand-mist">{{ __('Preset scripts') }}</p>
                        ...
                        >
                            <div class="min-w-0 flex-1">
                                ...
                                @if (filled($preset['script'] ?? null))
                                    <p class="mt-0.5 max-w-2xl truncate font-mono text-xs text-brand-moss" title="{{ $preset['script'] }}">{{ Str::limit($preset['script'], 120) }}</p>
                                @endif
                            </div>
                            <button
                                type="button"
