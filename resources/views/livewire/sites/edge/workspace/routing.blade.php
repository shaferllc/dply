                        ...
                        <tbody class="divide-y divide-brand-ink/8 text-brand-ink">
                            @foreach ($dashboard_rewrites as $index => $rule)
                                <tr wire:key="dash-rewrite-{{ $index }}">
                                    <td class="px-3 py-2 font-mono break-all">{{ $rule['from'] }}</td>
                                    <td class="px-3 py-2 font-mono break-all">{{ $rule['to'] }}</td>
                                    <td class="px-3 py-2 text-right"><button type="button" wire:click="removeRewrite({{ $index }})" wire:confirm="{{ __('Remove this rewrite?') }}" class="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-white px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-rose-700 shadow-sm hover:bg-rose-50">{{ __('Remove') }}</button></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    ...
                    @foreach ($dashboard_headers as $index => $rule)
                        <li wire:key="dash-headers-{{ $index }}" class="rounded-lg border border-brand-ink/10 bg-brand-sand/20 px-3 py-2 text-xs">
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <button type="button" wire:click="removeHeaderRule({{ $index }})" wire:confirm="{{ __('Remove this header rule?') }}" class="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-white px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-rose-700 shadow-sm hover:bg-rose-50">{{ __('Remove') }}</button>
                            </div>
                        </li>
                    @endforeach
                </ul>
