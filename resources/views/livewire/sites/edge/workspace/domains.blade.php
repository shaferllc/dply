    ...
    <section class="dply-card overflow-hidden">
        <div class="flex items-start gap-3 border-b border-brand-ink/10 bg-brand-sand/20 px-6 py-5 sm:px-7">
            ...
            <a
                ...
                class="inline-flex shrink-0 items-center gap-1 rounded-lg border border-brand-ink/15 bg-white px-2.5 py-1 font-mono text-2xs font-semibold uppercase tracking-wide text-brand-ink hover:bg-brand-sand/40"
            >
        ...
        <div class="px-6 py-4 sm:px-8">
            <div class="flex items-baseline justify-between gap-2">
                <h4 class="text-xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('From :file', ['file' => $sourcePath]) }}</h4>
                <span class="inline-flex items-center gap-1 rounded-full bg-brand-sand/60 px-2 py-0.5 font-mono text-2xs font-semibold uppercase tracking-wide text-brand-moss">{{ __('Repo-managed') }}</span>
            </div>
            @if ($repoDomains !== [])
                <div class="mt-2 rounded-lg border border-brand-ink/10 p-3">
                    ...
                    <p class="mt-2 text-xs text-brand-mist">{{ __('Auto-attached on every deploy. Removing a hostname from :file does NOT detach — detaches are explicit only.', ['file' => $sourcePath]) }}</p>
                </div>
            @else
                ...
                <pre class="mt-3 overflow-x-auto rounded-lg bg-brand-ink/95 px-4 py-3 font-mono text-xs leading-relaxed text-brand-sand"><code>domains:
  - "www.example.com"
