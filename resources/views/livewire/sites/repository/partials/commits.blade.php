<section>
    ...
    @if ($currentRepositoryUrl !== '')
        <div class="border-b border-brand-ink/10">
            <div class="flex flex-wrap items-start justify-between gap-x-4 gap-y-3 border-b border-brand-ink/10 bg-brand-sand/20 px-5 py-4 sm:px-6">
            ...
            <div>
                @if (! empty($commitsResult['notice']))
                    ...
                    <div class="mx-5 my-4 mb-2 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 sm:mx-6">
                ...
                @if (! ($commitsResult['ok'] ?? false))
                    <div class="mx-5 my-4 flex flex-col gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-900 sm:mx-6 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
                ...
                @else
                    <ul role="list" class="divide-y divide-brand-ink/10">
                        @foreach ($commitsFiltered as $commit)
                            ...
                            <li class="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-4 transition-colors hover:bg-brand-sand/15 sm:px-6" wire:key="commit-{{ $commit['sha'] }}">
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <code class="rounded-md bg-brand-sand/50 px-2 py-0.5 font-mono text-xs font-semibold text-brand-ink">{{ $commit['short_sha'] }}</code>
                                        @if ($isDeployed)
                                            <span class="inline-flex items-center gap-1 rounded-full bg-brand-sage/15 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-brand-forest ring-1 ring-brand-sage/20">
                                                <span class="inline-flex h-1.5 w-1.5 rounded-full bg-brand-sage"></span>
                                                {{ __('Deployed') }}
                                            </span>
                                        @endif
                                    </div>
                                    <p class="mt-1.5 truncate text-sm font-medium text-brand-ink">{{ $commit['message'] }}</p>
                    ...
                    @if ($commitsHasMore || $commitsPageNum > 1)
                        <div class="flex items-center justify-between gap-3 border-t border-brand-ink/10 px-5 py-4 sm:px-6">
                            <button
                                type="button"
                ...
                @if (! empty($commitsResult['account']['label']))
                    ...
                    <p class="flex flex-wrap items-center gap-1.5 border-t border-brand-ink/10 px-5 py-3.5 text-xs text-brand-moss sm:px-6">
