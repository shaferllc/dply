<div>
    @if ($this->canDeploy)
        ...
        >
            ...
            {{-- Sync: deploy this site together with its repo/worker peers.
                 Only shown when there's actually a peer to sync with. --}}
            @if ($hasSyncPeers)
                <button
                    type="button"
                ...
                >
                    ...
                    @else
                        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4">
                            <p class="mb-3 text-xs leading-relaxed text-brand-moss">{{ __('Ship this site and its repo/worker peers in one go. Each is deployed in parallel, exactly like its own Deploy button.') }}</p>
                        ...
                        <div class="border-t border-brand-ink/10 bg-brand-sand/20 px-5 py-4">
                            <button
                                type="button"
                                wire:click="deploySelected"
                                wire:loading.attr="disabled"
                                wire:target="deploySelected"
                                @disabled(count($syncSelected) === 0)
                                class="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand-ink px-4 py-2 text-sm font-semibold text-brand-cream shadow-sm transition-colors hover:bg-brand-forest disabled:opacity-50"
                            >
                                <x-heroicon-o-rocket-launch class="h-4 w-4" wire:loading.remove wire:target="deploySelected" />
                                <span wire:loading wire:target="deploySelected"><x-spinner variant="cream" size="sm" /></span>
                                {{ __('Deploy selected (:n)', ['n' => count($syncSelected)]) }}
                            </button>
                        </div>
                    @endif
                </div>
