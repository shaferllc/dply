...
@endif
        ...
        <div @class([
            ...
            <div class="flex flex-wrap items-center gap-1.5">
                <span class="text-2xs font-semibold uppercase tracking-[0.12em] text-brand-mist">{{ __('Preset') }}</span>
                @foreach ($this->presets() as $key => $preset)
                    <button type="button"
                        ...
                        class="rounded-md border border-brand-ink/15 bg-white px-2 py-0.5 text-xs font-semibold text-brand-ink hover:border-brand-forest/40 hover:bg-brand-sand/40">
                        {{ $preset['label'] }}
                    </button>
                @endforeach
            </div>
                ...
                <div class="{{ $stripPad }}">
                    @if (! empty($locked))
                        ...
                        <div class="mb-2 space-y-1">
                                    ...
                                    @if ($editing_step_id === (string) $step->id)
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <input type="text" wire:model="editing_step_command" spellcheck="false"
                                                ...
                                                class="min-w-0 flex-1 rounded-md border border-brand-ink/15 bg-white px-2 py-0.5 font-mono text-xs text-brand-ink focus:border-brand-forest focus:ring-brand-forest" autofocus>
                                            <button type="button" wire:click="saveStep"
                                                class="shrink-0 rounded-md bg-brand-forest px-2 py-0.5 text-2xs font-semibold text-white hover:bg-brand-forest/90">{{ __('Save') }}</button>
                                            <button type="button" wire:click="cancelStepEdit"
                                                class="shrink-0 rounded-md border border-brand-ink/15 bg-white px-2 py-0.5 text-2xs font-semibold text-brand-moss hover:bg-brand-sand/40">{{ __('Cancel') }}</button>
                                        </div>
                                        @error('editing_step_command')
                                            <p class="mt-1 text-2xs font-medium text-rose-600">{{ $message }}</p>
                                        @enderror
                                    @else
                                        <div class="flex items-center gap-1.5">
                                            <x-heroicon-m-lock-closed class="h-3 w-3 shrink-0 text-brand-mist" />
                                            <span class="shrink-0 text-xs font-semibold text-brand-ink">{{ $step->pillLabel() }}</span>
                                            <span class="min-w-0 flex-1 truncate font-mono text-2xs text-brand-mist">{{ $step->commandFor() }}</span>
                                            <span @class([
                                                'shrink-0 rounded-full px-1.5 py-0.5 text-3xs font-bold uppercase tracking-wide',
                            ...
                            @endforeach
                            <p class="text-2xs text-brand-mist">{{ __('Your commands below run after these.') }}</p>
                        </div>
                    @endif
                    ...
                    @if ($phase === 'restart')
                            ...
                            <div class="mb-2 rounded-md border border-brand-ink/10 bg-brand-sand/15 px-3 py-2" x-data="{ managed: @js($managed_restart_enabled), fpmStrategy: @js($php_fpm_strategy) }">
                                <label class="flex cursor-pointer items-start gap-2">
                                    ...
                                    <span class="min-w-0">
                                        ...
                                        <span class="mt-0.5 block text-xs text-brand-moss">{{ $managedRestart['label'] }}</span>
                                        <ul class="mt-1 space-y-0.5" :class="{ 'opacity-40': ! managed }">
                                            @foreach ($managedRestart['items'] as $item)
                                                <li class="flex items-start gap-1 text-xs text-brand-ink">
                                                    <x-heroicon-m-arrow-path class="mt-0.5 h-3 w-3 shrink-0 text-brand-forest" />
                                                    <span>{{ $item }}</span>
                                                </li>
                                            @endforeach
                                        </ul>
                                        <span x-show="! managed" x-cloak class="mt-1.5 flex items-start gap-1 text-xs font-medium text-amber-700">
                                ...
                                @if ($managedRestart['fpm_strategy_selectable'] ?? false)
                                    <div class="mt-2 border-t border-brand-ink/10 pt-2 pl-[26px]" x-show="managed" x-cloak>
                                        <label for="deploy-php-fpm-strategy" class="text-xs font-semibold text-brand-ink">{{ __('PHP-FPM & OPcache') }}</label>
                                        <select id="deploy-php-fpm-strategy" wire:model="php_fpm_strategy" x-on:change="fpmStrategy = $event.target.value"
                                            class="mt-1 block w-full max-w-md rounded-md border-brand-ink/15 bg-white py-1 text-xs text-brand-ink focus:border-brand-forest focus:ring-brand-forest">
                                            @foreach ($phpFpmStrategyOptions as $value => $label)
                                                <option value="{{ $value }}">{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        <p x-show="fpmStrategy === 'flush'" x-cloak class="mt-1 text-xs text-brand-moss">
                                            {{ __('Zero-downtime: FPM reload + OPcache flush; falls back to FPM restart if flush fails.') }}
                                        </p>
                                        <p x-show="fpmStrategy === 'flush_only'" x-cloak class="mt-1 flex items-start gap-1 text-xs font-medium text-amber-700">
                                            <x-heroicon-m-exclamation-triangle class="mt-0.5 h-3.5 w-3.5 shrink-0" />
                                            <span>{{ __('Never restarts FPM. If flush fails, previous release may stay live until a manual restart.') }}</span>
                                        </p>
                                        <p x-show="fpmStrategy === 'restart'" x-cloak class="mt-1 flex items-start gap-1 text-xs font-medium text-amber-700">
                                            <x-heroicon-m-exclamation-triangle class="mt-0.5 h-3.5 w-3.5 shrink-0" />
                        ...
                        @else
                            <p class="mb-2 rounded-md border border-brand-ink/10 bg-brand-sand/15 px-3 py-1.5 text-xs text-brand-moss">{{ $managedRestart['label'] }}</p>
                        @endif
                    @endif
        ...
        <div class="{{ $strip }}" x-data="{ zd: @js($atomic_release) }">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-brand-ink/10 {{ $isEmbedded ? 'bg-brand-sand/15 px-5 py-2 sm:px-6' : 'px-4 py-3 sm:px-5' }}">
                <div class="min-w-0">
                    ...
                    <p class="mt-0.5 text-xs text-brand-moss">{{ __('Shell at before/after clone and activate.') }}</p>
                </div>
            ...
            <div class="{{ $stripPad }}">
                @if ($hooks->isEmpty())
                    <p class="rounded-md border border-dashed border-brand-ink/15 bg-brand-sand/15 px-3 py-3 text-center text-xs text-brand-moss">
                    ...
                    <ul class="divide-y divide-brand-ink/10">
                        @foreach ($hooks as $hook)
                            ...
                            <li class="flex items-center gap-2 py-2" wire:key="hook-{{ $hook->id }}"
                                ...
                                <span class="shrink-0 rounded-full bg-brand-sage/15 px-1.5 py-0.5 text-2xs font-bold uppercase tracking-wide text-brand-forest">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="truncate text-xs font-semibold text-brand-ink">
                                        ...
                                        @if ($isMaintenanceHook)
                                            <span x-cloak x-show="zd"
                                                class="ml-1 inline-flex items-center rounded-full bg-amber-100 px-1.5 py-0.5 align-middle text-3xs font-bold uppercase tracking-wide text-amber-800"
                                    ...
                                    @if ($hook->hook_kind === $shellKind && trim((string) $hook->script) !== '')
                                        <p class="truncate font-mono text-2xs text-brand-mist">{{ \Illuminate\Support\Str::limit(trim((string) $hook->script), 80) }}</p>
                                    @endif
                                </div>
                                @if ($editable)
                                ...
                                @else
                                    <span class="shrink-0 rounded-full bg-brand-ink/5 px-1.5 py-0.5 text-3xs font-bold uppercase tracking-wide text-brand-moss" title="{{ __('Edit in the visual builder') }}">{{ __('Builder') }}</span>
                                @endif
                                <button type="button"
        ...
        <div x-data="{ stepId: null, stepLabel: '' }"
            ...
            <div class="border-b border-brand-ink/10 px-6 py-5">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-sage">{{ __('Remove step') }}</p>
        ...
        <div x-data="{ hookId: null, hookLabel: '' }"
            ...
            <div class="border-b border-brand-ink/10 px-6 py-5">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-sage">{{ __('Remove hook') }}</p>
                        <h2 class="mt-0.5 text-base font-semibold text-brand-ink">
        ...
        <div x-data="{ presetKey: null, presetLabel: '' }"
            ...
            <div class="border-b border-brand-ink/10 px-6 py-5">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-sage">{{ __('Preset') }}</p>
                        <h2 class="mt-0.5 text-base font-semibold text-brand-ink">
    ...
    <x-modal name="deploy-script-catalog" maxWidth="lg" overlayClass="bg-brand-ink/40" focusable>
        ...
        >
            <div class="flex items-start justify-between gap-3 border-b border-brand-ink/10 px-6 py-4">
                <div class="min-w-0">
                    <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-sage">{{ __('Insert command') }}</p>
                    <h2 class="mt-0.5 text-base font-semibold text-brand-ink">
                    ...
                    <template x-for="(item, i) in items()" :key="phase + '-' + i">
                        <li class="flex items-center justify-between gap-3 py-2.5">
                            <div class="min-w-0">
                                ...
                                <p class="truncate font-mono text-2xs text-brand-mist" x-text="item.command"></p>
                            </div>
    ...
    <x-modal name="deploy-script-hook" maxWidth="lg" overlayClass="bg-brand-ink/40" focusable>
        <form wire:submit="saveHook">
            <div class="border-b border-brand-ink/10 px-6 py-4">
                <p class="text-xs font-semibold uppercase tracking-[0.16em] text-brand-sage">{{ __('Deploy hook') }}</p>
