...
<div class="border-b border-brand-ink/10">
    <div class="{{ $stripHead }} flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <p class="min-w-0 flex-1 truncate text-[11px] text-brand-mist" title="{{ __('Point your hostname at this function. dply writes the CNAME when it manages the zone; otherwise you publish the target and verify.') }}">
    ...
    <form wire:submit.prevent="addCustomDomain" class="{{ $panelPad }} flex flex-col gap-2 sm:flex-row sm:items-end">
        <label class="min-w-0 flex-1 text-sm">
            <span class="block text-[10px] font-semibold uppercase tracking-wide text-brand-mist">{{ __('Hostname') }}</span>
            <input
                type="text"
...
<div class="border-b border-brand-ink/10">
    <div class="{{ $stripHead }} flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <span class="h-4 w-px shrink-0 bg-brand-ink/10" aria-hidden="true"></span>
        <p class="min-w-0 flex-1 truncate text-[11px] text-brand-mist">{{ __('Mode + live DNS state · manual rows show the CNAME target') }}</p>
        <span class="shrink-0 text-[11px] tabular-nums text-brand-moss">{{ trans_choice('{0} none|{1} :count domain|[2,*] :count domains', count($customDomains), ['count' => count($customDomains)]) }}</span>
    </div>
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-2">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                @endphp
                                <span class="inline-flex items-center rounded-full {{ $statusClasses }} px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em]">{{ $status }}</span>
                                <span class="inline-flex items-center rounded-full bg-brand-sand/40 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-brand-moss">{{ $domain['mode'] }}</span>
                            </div>
                            ...
                            @if (! empty($domain['error']))
                                <p class="mt-1 text-[11px] text-rose-700">{{ $domain['error'] }}</p>
                            @endif
                            @if (! empty($domain['verified_at']))
                                <p class="mt-0.5 text-[10px] text-brand-mist">{{ __('Last checked:') }} {{ \Illuminate\Support\Carbon::parse($domain['verified_at'])->diffForHumans() }}</p>
                            @endif
                        </div>
                        <div class="flex shrink-0 flex-wrap items-center gap-1.5">
                            @if (($domain['mode'] ?? null) === 'manual')
                                <button
                                    ...
                                    class="inline-flex items-center gap-1 rounded-lg border border-brand-ink/15 bg-white px-2 py-1 text-[11px] font-semibold text-brand-ink shadow-sm hover:bg-brand-sand/40 disabled:cursor-wait disabled:opacity-60"
                                >
...
<div class="{{ $panelPad }} bg-brand-sand/15 text-[11px] leading-relaxed text-brand-moss">
