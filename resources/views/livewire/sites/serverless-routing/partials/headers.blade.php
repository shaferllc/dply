...
<div class="border-b border-brand-ink/10">
    <div class="{{ $stripHead }} flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <p class="min-w-0 flex-1 truncate text-xs text-brand-mist" title="{{ __('Merged onto every proxied response. Content-Type, Cache-Control, and Location are reserved.') }}">
    ...
    <div class="{{ $panelPad }} space-y-2.5">
        <form wire:submit.prevent="addHeader" class="grid gap-2 sm:grid-cols-12 sm:items-end">
            <label class="sm:col-span-4 text-sm">
                <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Name') }}</span>
                <input
                    type="text"
            ...
            <label class="sm:col-span-7 text-sm">
                <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Value') }}</span>
                <input
                    type="text"
...
<div>
    <div class="{{ $stripHead }} flex flex-wrap items-center gap-x-2 gap-y-1">
        ...
        <span class="h-4 w-px shrink-0 bg-brand-ink/10" aria-hidden="true"></span>
        <p class="min-w-0 flex-1 truncate text-xs text-brand-mist" title="{{ __('When enabled, the proxy short-circuits OPTIONS preflights and decorates responses.') }}">
    ...
    <form wire:submit.prevent="saveCors" class="{{ $panelPad }} space-y-3">
        <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="min-w-0">
                ...
                <p class="text-xs text-brand-moss">{{ __('Disable if your function emits CORS headers itself.') }}</p>
            </div>
            <x-toggle-switch
        ...
        <label class="block text-sm">
            <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Allowed origins (comma-separated, or *)') }}</span>
            <input
                type="text"
        ...
        <label class="block text-sm">
            <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Allowed methods') }}</span>
            <input
                type="text"
        ...
        <label class="block text-sm">
            <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Allowed request headers') }}</span>
            <input
                type="text"
        ...
        <div class="grid gap-2 sm:grid-cols-2 sm:items-end">
            ...
            <label class="block text-sm">
                <span class="block text-2xs font-semibold uppercase tracking-wide text-brand-mist">{{ __('Preflight max age (seconds)') }}</span>
                <input
                    type="number"
                    min="0"
