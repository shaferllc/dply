<div class="mx-auto max-w-3xl px-6 py-10">
    ...
    @if (! $hasDoCredential)
        <div class="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 text-sm text-amber-900">
            <p class="font-semibold">{{ __('No DigitalOcean credential connected') }}</p>
            <p class="mt-1">{{ __('Managed databases run on DigitalOcean. Connect a DigitalOcean credential first — that\'s the cloud account dply uses to provision the database cluster.') }}</p>
            <p class="mt-3">
