<div class="contents">
    ...
    <div class="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        @if ($error)
            ...
            <div class="mt-6">
                <a href="{{ route('live.servers.index') }}" wire:navigate class="text-sm font-semibold text-brand-forest hover:underline">
        ...
        @else
            <div class="flex flex-col items-center justify-center gap-3 text-center">
                ...
                <p class="text-sm font-semibold text-brand-ink">{{ __('Opening production server…') }}</p>
                <p class="max-w-md text-sm text-brand-moss">
                    {{ __('Loading this server from the Production API into your local workspace.') }}
                </p>
            </div>
        @endif
    </div>
