<div>
    <x-infrastructure-shell
        :title="__('Deploys')"
        :description="__('In-flight, failed, and stagnant deploy activity across every site.')"
        :section="__('Deploys')"
        icon="heroicon-o-rocket-launch"
    >
        <nav class="flex flex-wrap items-center gap-2 border-b border-brand-ink/10 px-5 py-3 sm:px-6" aria-label="{{ __('Deploy activity tabs') }}">
            <x-infrastructure-pill :active="$tab === 'running'" wire:click="setTab('running')">
                {{ __('Running') }} <span class="text-xs opacity-70">({{ $counts['running'] }})</span>
            </x-infrastructure-pill>
            <x-infrastructure-pill :active="$tab === 'failed-latest'" wire:click="setTab('failed-latest')">
                {{ __('Failed latest') }} <span class="text-xs opacity-70">({{ $counts['failed-latest'] }})</span>
            </x-infrastructure-pill>
            <x-infrastructure-pill :active="$tab === 'stale'" wire:click="setTab('stale')">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-brand-ink/10 text-sm">
                    <thead class="bg-brand-sand/30 text-left text-xs font-semibold uppercase tracking-[0.12em] text-brand-moss">
                        <tr>
                            <th class="px-5 py-3 sm:px-6">{{ __('Site') }}</th>
                            ...
                            <th class="px-5 py-3 sm:px-6">{{ __('Deploy ID') }}</th>
                        </tr>
                    </thead>
        ...
        @php
            $deploysCommand = match ($tab) {
                'running' => 'dply ops:running-deploys',
                'failed-latest' => 'dply ops:failed-deploys',
                default => 'dply ops:stale-deploys --days='.$staleDays,
            };
        @endphp
        <x-slot:footer>
