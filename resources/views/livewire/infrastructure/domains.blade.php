<div>
    ...
    >
        ...
        @if ($rows === [])
            <x-infrastructure-empty>{{ __('No domains match the current filters.') }}</x-infrastructure-empty>
        @else
                ...
                <table class="min-w-full divide-y divide-brand-ink/10 text-sm">
                    <thead class="bg-brand-sand/30 text-left text-xs font-semibold uppercase tracking-[0.12em] text-brand-moss">
                        <tr>
                            ...
                            <th class="px-5 py-3 sm:px-6">{{ __('Primary') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-brand-ink/5">
                        @foreach ($rows as $row)
                            <tr class="hover:bg-brand-sand/20">
                                <td class="px-5 py-3 font-mono text-xs text-brand-ink sm:px-6">{{ $row['hostname'] }}</td>
        ...
        <x-slot:footer>
            <x-cli-snippet :commands="[
                ['label' => __('List all'), 'command' => 'dply ops:domain-list'],
                ['label' => __('Find by hostname'), 'command' => 'dply ops:domain-find example.com'],
            ]" />
        </x-slot:footer>
    </x-infrastructure-shell>
</div>
