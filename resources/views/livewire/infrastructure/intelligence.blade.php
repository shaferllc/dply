    ...
    >
        ...
        <section class="border-b border-brand-ink/10 px-5 py-5 sm:px-6">
            <div class="grid gap-3 sm:grid-cols-3">
                <x-infrastructure-stat :label="__('Open')">
                ...
                <x-infrastructure-stat :label="__('Auto-resolved')">
                    <p class="mt-2 text-3xl font-semibold tabular-nums text-emerald-600">{{ $totals['resolved'] }}</p>
                </x-infrastructure-stat>
                <x-infrastructure-stat :label="__('Dismissed')">
        ...
        <div class="flex flex-wrap items-center gap-2 border-b border-brand-ink/10 px-5 py-3 sm:px-6">
            ...
            @foreach (['open' => __('Open'), 'resolved' => __('Resolved'), 'dismissed' => __('Dismissed'), 'all' => __('All')] as $value => $label)
                <x-infrastructure-pill :active="$showFilter === $value" wire:click="$set('showFilter', '{{ $value }}')">{{ $label }}</x-infrastructure-pill>
            @endforeach
            ...
            @foreach (['' => __('All'), 'slow_build' => __('Slow build'), 'tls_expiring' => __('TLS expiring'), 'env_drift' => __('Env drift')] as $value => $label)
                <x-infrastructure-pill :active="$ruleFilter === $value" wire:click="$set('ruleFilter', '{{ $value }}')">{{ $label }}</x-infrastructure-pill>
            @endforeach
        </div>

        @if ($alerts->isEmpty())
            @if ($showFilter === 'open' && $ruleFilter === '')
                <x-infrastructure-empty :title="__('Nothing to act on.')">
            ...
            @else
                <x-infrastructure-empty>{{ __('No alerts match the current filters.') }}</x-infrastructure-empty>
            @endif
        @else
                ...
                @foreach ($alerts as $alert)
                    @php
                        $tone = match ($alert->severity) {
                            'danger' => ['strip' => 'bg-rose-50/40', 'dot' => 'bg-rose-500', 'pill' => 'bg-rose-100 text-rose-900 ring-rose-200'],
                            'warning' => ['strip' => 'bg-amber-50/40', 'dot' => 'bg-amber-500', 'pill' => 'bg-amber-100 text-amber-900 ring-amber-200'],
                            default => ['strip' => '', 'dot' => 'bg-brand-sage', 'pill' => 'bg-brand-sage/15 text-brand-forest ring-brand-sage/25'],
                        };
                    ...
                    @endphp
                    <li class="px-5 py-5 sm:px-6 {{ $tone['strip'] }}">
