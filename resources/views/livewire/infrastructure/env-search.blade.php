<div>
    <x-infrastructure-shell
        ...
        icon="heroicon-o-key"
    >
        <div class="flex flex-wrap items-end gap-3 border-b border-brand-ink/10 px-5 py-4 sm:px-6">
        ...
        @if (! $hasQuery)
            <x-infrastructure-empty>{{ __('Enter a key (or prefix) above to search.') }}</x-infrastructure-empty>
        @elseif ($rows === [])
            <x-infrastructure-empty>{{ __('No matches across the organization.') }}</x-infrastructure-empty>
        @else
                ...
                <table class="min-w-full divide-y divide-brand-ink/10 text-sm">
                    <thead class="bg-brand-sand/30 text-left text-xs font-semibold uppercase tracking-[0.12em] text-brand-moss">
                        <tr>
                            ...
                            <th class="px-5 py-3 sm:px-6">{{ $reveal ? __('Value') : __('Value (masked)') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-brand-ink/5">
                        @foreach ($rows as $row)
                            <tr class="hover:bg-brand-sand/20">
                                <td class="px-5 py-3 text-brand-ink sm:px-6">{{ $row['site']->name }}</td>
                                <td class="px-4 py-3 text-brand-moss">{{ $row['environment'] }}</td>
                                <td class="px-4 py-3 font-mono text-xs text-brand-ink">{{ $row['key'] }}</td>
                                <td class="px-5 py-3 font-mono text-xs text-brand-moss sm:px-6">{{ $row['value'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
            <p class="border-t border-brand-ink/10 px-5 py-3 text-xs text-brand-mist sm:px-6">{{ trans_choice('{1} 1 match|[2,*] :count matches', count($rows), ['count' => count($rows)]) }}</p>
        @endif
