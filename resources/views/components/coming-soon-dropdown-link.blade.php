<div class="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-start text-sm font-medium leading-5 cursor-default select-none" aria-disabled="true" title="{{ __('Coming soon') }}">
    ...
    @endisset
    <span class="min-w-0 flex-1 text-brand-moss/55">{{ $slot }}</span>
    <span class="shrink-0 rounded-full bg-brand-gold/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-brand-gold ring-1 ring-inset ring-brand-gold/25">{{ __('Soon') }}</span>
</div>
