...
>
    <div @class([
        ...
        <div class="pointer-events-none absolute -end-16 -top-20 h-56 w-56 rounded-full bg-amber-400/10 blur-3xl" aria-hidden="true"></div>
        ...
        <div class="relative mt-5 space-y-2.5 font-mono text-[11px] leading-relaxed sm:text-xs">
            ...
            <div class="rounded-lg border border-white/8 bg-white/[0.03] px-3 py-2.5">
                <p class="text-amber-200/90"><span class="text-slate-500">{{ __('warning') }}</span> · {{ __('Disk use above 85%') }}</p>
                <p class="mt-1 text-slate-500">{{ __('Root volume at 87% — headroom shrinking on /var.') }}</p>
            </div>
            <div class="rounded-lg border border-white/8 bg-white/[0.03] px-3 py-2.5">
                <p class="text-sky-200/90"><span class="text-slate-500">{{ __('info') }}</span> · {{ __('Unattended upgrades off') }}</p>
                <p class="mt-1 text-slate-500">{{ __('Security patches may lag until you enable auto-updates.') }}</p>
            </div>
        </div>
    ...
    <div @class([
        ...
        <div @class([
            ...
            <div @class(['max-w-md', 'text-center sm:text-left' => ! $compact])>
                <div @class([
                    'inline-flex items-center justify-center rounded-xl bg-amber-50 text-amber-800 ring-1 ring-amber-200/80',
                    'h-11 w-11' => ! $compact,
                    'h-9 w-9' => $compact,
                ])>
                    <x-heroicon-o-light-bulb @class([
                        'shrink-0',
                        'h-6 w-6' => ! $compact,
                        'h-5 w-5' => $compact,
                    ]) aria-hidden="true" />
                ...
                ])>{{ __('Server insights') }}</h2>
                <p @class([
                    'leading-6 text-brand-moss',
                    'mt-2 text-sm sm:text-[15px]' => ! $compact,
                    'mt-1.5 text-xs' => $compact,
                ])>
                    {{ __('Automated health checks, prioritized findings, optional one-click fixes over SSH, and notification routing — without leaving the server workspace.') }}
                </p>
            </div>
        </div>
        ...
        <ul @class([
            ...
            @foreach ([
                ['icon' => 'magnifying-glass', 'title' => __('Continuous checks'), 'body' => __('Config drift, package versions, log signals, and resource pressure on a background cadence.')],
                ['icon' => 'bolt', 'title' => __('Apply fix actions'), 'body' => __('Safe, auditable SSH fixes with backup + revert when a check offers a remediation path.')],
                ['icon' => 'bell-alert', 'title' => __('Alert routing'), 'body' => __('Wire critical findings to your existing notification channels and org digests.')],
                ['icon' => 'chart-bar', 'title' => __('Fleet rollup'), 'body' => __('Open-finding counts on the server index and overview shortcuts when checks are active.')],
            ] as $feature)
                <li @class([
        ...
        <div @class([
            ...
            <p @class(['text-brand-moss', 'text-sm' => ! $compact, 'text-xs' => $compact])>
                {{ __('We will enable Insights for your org when the findings workspace ships.') }}
            </p>
            <span @class([
