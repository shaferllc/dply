...
>
    ...
    <div @class([
        ...
        <div class="relative flex items-start justify-between gap-3">
            ...
            <span class="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-2xs font-semibold uppercase tracking-[0.16em] text-sky-200/90">
        ...
        <div class="relative mt-4 font-mono text-xs leading-relaxed sm:text-xs">
            <p class="text-slate-500">{{ __('# Docker inspector preview — :host', ['host' => $hostLabel]) }}</p>
            <p class="mt-3 text-sky-300/90">{{ __('~ $ docker ps') }}</p>
            <p class="text-slate-400">{{ __('CONTAINER   IMAGE             STATUS') }}</p>
            <p class="text-slate-400">{{ __('a1b2c3d4    nginx:alpine      Up 4 days') }}</p>
            <p class="text-slate-400">{{ __('e5f6g7h8    postgres:16       Up 4 days (healthy)') }}</p>
            <p class="mt-3 text-emerald-400/90">{{ __('3 images · 2 volumes · 1 network · 1 compose project') }}</p>
        ...
        @if ($server)
            <div class="relative mt-4 inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300">
                ...
                <span>{{ __('The Docker inspector will live on :server when it ships.', ['server' => $server->name]) }}</span>
            </div>
        @endif
    </div>
    ...
    <div @class([
        ...
        <div @class([
            ...
            <div @class(['max-w-md', 'text-center sm:text-left' => ! $compact])>
                ...
                <p @class([
                    ...
                    {{ __('Inspect and manage the Docker Engine on this server over SSH — browse containers, images, volumes, networks, and compose projects, then start, stop, prune, or pull without leaving Dply.') }}
                </p>
            </div>
        </div>
        ...
        <ul @class([
            ...
            @foreach ([
                ['icon' => 'cube', 'title' => __('Containers & logs'), 'body' => __('Start, stop, restart, inspect, and tail logs for any container.')],
                ['icon' => 'photo', 'title' => __('Images & pulls'), 'body' => __('Pull new images, remove unused ones, and prune dangling layers.')],
                ['icon' => 'circle-stack', 'title' => __('Volumes & networks'), 'body' => __('Browse volumes and networks, and reclaim space with prune.')],
                ['icon' => 'rectangle-stack', 'title' => __('Compose & cleanup'), 'body' => __('See compose projects and run safe system-wide cleanup.')],
            ] as $feature)
                <li @class([
        ...
        <div @class([
            ...
            <p @class([
                ...
                {{ __('We will enable this for your org when the Docker inspector ships.') }}
            </p>
            <span @class([
