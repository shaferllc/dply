...
>
    ...
    <div @class([
        ...
        <div class="relative mt-4 font-mono text-[11px] leading-relaxed sm:text-xs">
            <p class="text-slate-500">{{ __('# Maintenance window preview — :host', ['host' => $hostLabel]) }}</p>
            <p class="mt-3 text-amber-300/90">{{ __('maintenance   ON  ·  until 02:00 (in 45m)') }}</p>
            <p class="text-slate-400">{{ __('eligible      6 sites suspended  ·  2 excluded') }}</p>
            <p class="text-slate-400">{{ __('serving       static "We\'ll be right back" page') }}</p>
            <p class="text-emerald-400/90">{{ __('webserver     configs queued') }}</p>
        ...
        @if ($server)
            <div class="relative mt-4 inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[11px] text-slate-300">
                ...
                <span>{{ __('Maintenance windows will live on :server when it ships.', ['server' => $server->name]) }}</span>
            </div>
        @endif
    </div>
    ...
    <div @class([
        ...
        <div @class([
            ...
            <div @class(['max-w-md', 'text-center sm:text-left' => ! $compact])>
                ...
                <p @class([
                    ...
                    {{ __('Put every eligible site on this server into maintenance with one toggle — visitors see a friendly suspended page while you work, then resume them all just as fast.') }}
                </p>
            </div>
        </div>
        ...
        <ul @class([
            ...
            @foreach ([
                ['icon' => 'power', 'title' => __('One-toggle suspend'), 'body' => __('Suspend and resume all eligible VM sites on the host at once.')],
                ['icon' => 'clock', 'title' => __('Timed windows'), 'body' => __('Set an end time and maintenance auto-clears when it passes.')],
                ['icon' => 'chat-bubble-left-right', 'title' => __('Visitor message'), 'body' => __('Show a shared, branded "be right back" page to visitors.')],
                ['icon' => 'globe-alt', 'title' => __('Site impact preview'), 'body' => __('See exactly which sites will be affected before you start.')],
            ] as $feature)
                <li @class([
        ...
        <div @class([
            ...
            <p @class([
                ...
                {{ __('We will enable this for your org when maintenance windows ship.') }}
            </p>
            <span @class([
