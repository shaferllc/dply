...
>
    ...
    <div @class([
        ...
        <div class="relative flex items-start justify-between gap-3">
            ...
            <span class="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-2xs font-semibold uppercase tracking-[0.16em] text-violet-200/90">
        ...
        <div class="relative mt-4 font-mono text-xs leading-relaxed sm:text-xs">
            ...
            <p class="text-slate-400">{{ __('deploy@dply      org           in sync') }}</p>
            <p class="text-amber-300/90">{{ __('contractor-1     session       expires in 3h') }}</p>
        ...
        @if ($server)
            <div class="relative mt-4 inline-flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-300">
                <x-heroicon-o-server-stack class="h-3.5 w-3.5 shrink-0 text-violet-300/80" aria-hidden="true" />
                <span>{{ __('The access graph will live on :server when it ships.', ['server' => $server->name]) }}</span>
            </div>
        @endif
    </div>
    ...
    <div @class([
        ...
        <ul @class([
            ...
            @foreach ([
                ['icon' => 'key', 'title' => __('Full key inventory'), 'body' => __('Every authorized key across profile, org, team, and server-local sources.')],
                ['icon' => 'arrow-path', 'title' => __('Sync & drift status'), 'body' => __('See which keys are in sync with authorized_keys and which drifted.')],
                ['icon' => 'clock', 'title' => __('Time-boxed sessions'), 'body' => __('Grant contractors temporary SSH access that auto-revokes on expiry.')],
                ['icon' => 'shield-check', 'title' => __('Review & revoke'), 'body' => __('Audit access at a glance and revoke any session in one click.')],
            ] as $feature)
                <li @class([
        ...
        <div @class([
            ...
            <p @class([
                ...
                {{ __('We will enable this for your org when the access graph ships.') }}
            </p>
            <span @class([
