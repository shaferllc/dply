...
@if ($items->isNotEmpty() || $dismissedCount > 0)
    ...
    <details class="group rounded-xl border border-indigo-200 bg-gradient-to-b from-indigo-50/90 to-white" @if ($items->isNotEmpty()) open @endif>
        <summary class="flex cursor-pointer list-none flex-wrap items-center gap-2 px-3 py-2">
            ...
            <span class="shrink-0 text-xs font-semibold uppercase tracking-[0.16em] text-indigo-700">{{ __('Suggested processes') }}</span>
            @if ($items->isNotEmpty())
                <span class="inline-flex shrink-0 items-center rounded-full bg-indigo-100 px-2 py-0.5 text-2xs font-semibold uppercase tracking-wide text-indigo-900 ring-1 ring-indigo-200">
                    {{ $items->count() }}
                </span>
            @endif
            ...
            <ul class="divide-y divide-indigo-100 border-t border-indigo-100">
                @foreach ($items as $item)
                    ...
                    <li class="flex flex-wrap items-center gap-x-2 gap-y-1 px-3 py-1.5">
                        <span @class([
                            'inline-flex shrink-0 rounded-full px-1.5 py-0.5 text-3xs font-semibold uppercase tracking-wide',
                        ...
                        @if (($item['command'] ?? '') !== '')
                            <code class="shrink-0 rounded bg-white/70 px-1 font-mono text-2xs text-brand-moss ring-1 ring-inset ring-indigo-100">{{ $item['command'] }}</code>
                        @endif
                        <span class="min-w-0 flex-1 truncate text-xs text-indigo-950/80" title="{{ $item['reason'] }}">{{ $item['reason'] }}</span>
                        ...
                        <span class="flex shrink-0 items-center gap-1">
                            @if ($mode === 'interactive' && ! $isScheduler && ($item['preset'] ?? null))
                                <button
                                    ...
                                    class="inline-flex items-center gap-1 rounded-md border border-indigo-300 bg-white px-2 py-0.5 text-xs font-semibold text-indigo-700 shadow-sm transition-colors hover:bg-indigo-50"
                                >
        ...
        @if ($dismissedCount > 0)
            <div class="flex items-center justify-between gap-2 border-t border-indigo-100 px-3 py-1.5">
                <span class="text-xs text-indigo-950/70">
