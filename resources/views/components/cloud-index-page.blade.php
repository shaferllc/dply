...
@php
    ...
    $sourceCreateUrl = route('cloud.create', ['mode' => 'source']);
    $imageCreateUrl = route('cloud.create', ['mode' => 'image']);
...
<div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
    ...
    @elseif ($showStarter)
        <x-cloud-starter
            :create-url="$sourceCreateUrl"
            :image-create-url="$imageCreateUrl"
            :databases-url="$showDatabasesAction ? $databasesUrl : null"
            :digital-ocean-credentials-url="$digitalOceanCredentialsUrl"
            :aws-credentials-url="$awsCredentialsUrl"
            :show-create-action="$showCreateAction"
            :show-databases-action="$showDatabasesAction"
            :show-credential-ctas="$showCredentialCtas"
        />
    @else
        <x-profile-shell
            :title="__('Cloud apps')"
                ...
                @else
                    ...
                    <div class="flex flex-col items-center justify-center px-5 py-16 text-center sm:px-6" aria-labelledby="cloud-empty-heading">
                        ...
                        <h2 id="cloud-empty-heading" class="mt-4 text-sm font-semibold text-brand-ink">
                            {{ $isProductionSurface ? __('No production Cloud apps') : __('No apps yet') }}
                        </h2>
                        <p class="mt-1 max-w-md text-sm leading-relaxed text-brand-moss">
                            {{ $isProductionSurface
                                ? __('The connected control plane returned no Cloud apps for this organization.')
                                : __('Nothing deployed to Cloud in this organization yet.') }}
                        </p>
                    </div>
                @endif
            @else
