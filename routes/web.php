...
Route::middleware(['auth', 'verified', 'org'])->group(function () {
    ...
    // WordPress themes/plugins from their own repos now live as the Git tab of
    // the WordPress section — a standalone page stranded the operator with no
    // way back to the other WordPress tabs. The old URL still resolves.
    Route::get('servers/{server}/sites/{site}/git-sources', fn (Server $server, Site $site) => redirect()->route('sites.show', [
        'server' => $server,
        'site' => $site,
        'section' => 'wordpress',
        'wp' => 'git',
    ]))->name('sites.git-sources');
