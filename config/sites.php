...
return [
    ...
    */
    ...
    /*
    | A deployed function POSTs its per-request visit logs to dply's ingest
    | endpoint. The function runs on DigitalOcean and must reach dply over
    | the public internet — the URL is built from DPLY_PUBLIC_APP_URL
    | (config dply.public_app_url), the same public/tunnel URL server-metrics
    | callbacks use. Unset (local dev) means the function skips reporting.
    */
    'function_log_ingest_per_minute' => (int) env('DPLY_FUNCTION_LOG_INGEST_PER_MINUTE', 1000),
