...
/**
 * Abstract Base Class for View/Show Actions
 *
 * This abstract class provides a comprehensive baseline for all view/show/read actions
 * in the application. It includes essential concerns and useful optional concerns
 * that most view actions will benefit from.
 *
 * Note: View actions are read-only operations, so they don't include:
 * - AsValidated (no input to validate)
 * - AsTransaction (read-only, no database writes)
 * - AsUpdatable (no changes to track)
 * - AsReversible (can't undo a view)
 * - AsRetry (typically not needed for read operations)
 *
 * Essential Concerns Included:
 * - AsAuthenticated: Authentication requirement
 * - AsAuthorized: Permission checks (default: 'view')
 * - AsAuditable: Audit trail logging (tracks what was viewed)
 ...
 * - AsWebhook: External system notifications (e.g., analytics)
 * - AsEvent: Laravel event dispatching (e.g., view tracking)
 * - AsBroadcast: Real-time updates (e.g., live data)
 *
 * Usage:
 *
 * ```php
 * class ShowTag extends AbstractView
 * {
 *     public function handle(Tag $tag): Tag
 *     {
 *         // Your view logic here
 *         return $tag->load('relationships');
 *     }
 * }
 * ```
 *
 * Override Methods:
 * - `handle(...$arguments)` - REQUIRED: Implement your view logic
 * - `getAuthorizationAbility()` - Customize permission check (default: 'view')
...
abstract class AbstractView
{
     ...
     * the actual view logic. Subclasses can define their own
    ...
    protected function getTraceName(): string
    {
        ...
        return $module ? "{$module}.view" : strtolower($className);
    }
    ...
    protected function getBroadcastEventName(): string
    {
        ...
        return $module && $resource ? "{$module}.{$resource}.viewed" : 'resource.viewed';
    }
    ...
    {
        $className = class_basename(static::class);
        // Remove "Show", "View", or "Get" prefix if present
        $resource = preg_replace('/^(Show|View|Get)/', '', $className);
    ...
    /**
     * Dispatch this action as an event after successful view.
     *
     * Automatically dispatches the action as a Laravel event, allowing
     * other parts of the application to react to the view (e.g., analytics).
     *
     * @param  mixed  ...$arguments  The action arguments
     *
     * @example
     * // In your handle() method after viewing:
     * $tag = $tag->load('relationships');
     * $this->dispatchViewedEvent($tag);
     * return $tag;
     */
    protected function dispatchViewedEvent(...$arguments): void
    {
     ...
     * @param  mixed  ...$arguments  The action arguments
     */
    protected function dispatchViewedEventIf(bool $condition, ...$arguments): void
    {
     ...
     *
     * Returns a consistent response structure for successful views.
     * Note: This is a data structure helper, not an HTTP response.
     * Use AsResponse concern for HTTP responses.
     *
     * @param  mixed  $result  The viewed resource
     * @param  string  $message  Success message
     * @param  array<string, mixed>  $metadata  Additional metadata
     * @return array<string, mixed> Standardized response data
     */
    protected function successResponseData($result, string $message = 'Retrieved successfully', array $metadata = []): array
    {
        return [
            'success' => true,
            'message' => $message,
     ...
     */
    protected function canView(...$arguments): bool
    {
    ...
    /**
     * Ensure user has permission to view resource.
     *
     ...
     */
    protected function ensureCanView(...$arguments): void
    {
        if (! $this->canView(...$arguments)) {
     ...
     *
     * Extracts resource type from class name (e.g., "ShowTag" -> "tag").
     *
    ...
    /**
     * Load relationships on the resource.
     *
     * Convenience method for eager loading relationships.
     *
     * @param  mixed  $resource  The resource to load relationships on
     * @param  array<int, string>|string  $relationships  Relationships to load
     * @return mixed Resource with relationships loaded
     */
    protected function loadRelationships($resource, array|string $relationships)
    {
        if (method_exists($resource, 'load')) {
            return $resource->load($relationships);
        }
    ...
    /**
     * Perform pre-view tasks.
     *
     * Override this method to perform tasks before viewing.
     *
     * @param  mixed  $resource  The resource being viewed
     * @param  array<int, mixed>  $arguments  The action arguments
     */
    protected function beforeView($resource, array $arguments): void
    {
        // Override to perform pre-view tasks
        // e.g., increment view count, check access, etc.
    }

    /**
     * Perform post-view tasks.
     *
     * Override this method to perform tasks after viewing.
     *
     * @param  mixed  $result  The view result
     * @param  array<int, mixed>  $arguments  The action arguments
     */
    protected function afterView($result, array $arguments): void
    {
        // Override to perform post-view tasks
        // e.g., track analytics, send notifications, etc.
    }
}
