...
/**
 * Powers the "Notifications" sub-tab on the security-digest workspace: binds
 * notification channels to this server's server.security_digest.* events without
 * leaving the page.
 *
 * Security-digest events fire from {@see ServerSecurityDigestScanner}
 * (manual refresh + daily sweep), so this trait is subscription management only.
 * Mirrors {@see ManagesCertInventoryNotifications}.
 */
trait ManagesSecurityDigestNotifications
{
    ...
    {
        return ServerSecurityDigestNotificationKeys::eventKeys();
    }
    ...
    /**
     * server.security_digest.* event keys ticked in the add form. Seeded to all of
    ...
    public function mountManagesSecurityDigestNotifications(): void
    {
        $this->bootFeatureNotificationMatrix();

        $this->notif_event_keys = ServerSecurityDigestNotificationKeys::eventKeys();
    }

    public function addSecurityDigestNotificationSubscription(): void
    {
        $this->authorize('update', $this->server);

        $allowedKeys = ServerSecurityDigestNotificationKeys::eventKeys();
        ...
        $assignable = $this->assignableSecurityDigestNotificationChannels()
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();
        ...
        $this->toastSuccess($created > 0
            ? __('Routing :count security digest event(s) to :channel.', ['count' => $created, 'channel' => $channel->label])
            : __('Those events are already routed to :channel.', ['channel' => $channel->label]));
    }

    public function removeSecurityDigestNotificationSubscription(string $subscriptionId): void
    {
        $this->authorize('update', $this->server);
     ...
     */
    protected function assignableSecurityDigestNotificationChannels(): Collection
    {
     ...
     */
    protected function securityDigestNotificationSubscriptions(): Collection
    {
     ...
     */
    protected function securityDigestEventLabels(): array
    {
