...
/**
 * Powers the "Notifications" sub-tab on the maintenance workspace: binds
 * notification channels to this server's server.maintenance.* events without
 * leaving the page.
 *
 * Maintenance events fire from {@see ServerMaintenanceWindow},
 * so this trait is subscription management only. Mirrors
 * {@see ManagesCertInventoryNotifications}.
 */
trait ManagesMaintenanceNotifications
{
    ...
    {
        return ServerMaintenanceNotificationKeys::eventKeys();
    }
    ...
    /**
     * server.maintenance.* event keys ticked in the add form. Seeded to all of them
    ...
    public function mountManagesMaintenanceNotifications(): void
    {
        $this->bootFeatureNotificationMatrix();

        $this->notif_event_keys = ServerMaintenanceNotificationKeys::eventKeys();
    }

    public function addMaintenanceNotificationSubscription(): void
    {
        $this->authorize('update', $this->server);

        $allowedKeys = ServerMaintenanceNotificationKeys::eventKeys();
        ...
        $assignable = $this->assignableMaintenanceNotificationChannels()
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();
        ...
        $this->toastSuccess($created > 0
            ? __('Routing :count maintenance event(s) to :channel.', ['count' => $created, 'channel' => $channel->label])
            : __('Those events are already routed to :channel.', ['channel' => $channel->label]));
    }

    public function removeMaintenanceNotificationSubscription(string $subscriptionId): void
    {
        $this->authorize('update', $this->server);
        ...
        $sub = NotificationSubscription::query()
            ...
            ->whereIn('event_key', ServerMaintenanceNotificationKeys::eventKeys())
     ...
     */
    protected function assignableMaintenanceNotificationChannels(): Collection
    {
     ...
     */
    protected function maintenanceNotificationSubscriptions(): Collection
    {
     ...
     */
    protected function maintenanceEventLabels(): array
    {
