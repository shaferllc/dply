...
trait ManagesPatchNotifications
{
    ...
    /**
     * server.patches.* event keys ticked in the add form. Seeded to all of them so
    ...
    public function addPatchNotificationSubscription(): void
    {
        $this->authorize('update', $this->server);

        $allowedKeys = ServerPatchNotificationKeys::eventKeys();
        ...
        $assignable = $this->assignablePatchNotificationChannels()
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();
        ...
        $this->toastSuccess($created > 0
            ? __('Routing :count patch event(s) to :channel.', ['count' => $created, 'channel' => $channel->label])
            : __('Those events are already routed to :channel.', ['channel' => $channel->label]));
    }

    public function removePatchNotificationSubscription(string $subscriptionId): void
    {
        $this->authorize('update', $this->server);
        ...
        $sub = NotificationSubscription::query()
            ...
            ->whereIn('event_key', ServerPatchNotificationKeys::eventKeys())
     ...
     */
    protected function assignablePatchNotificationChannels(): Collection
    {
     ...
     */
    protected function patchNotificationSubscriptions(): Collection
    {
     ...
     */
    protected function patchEventLabels(): array
    {
