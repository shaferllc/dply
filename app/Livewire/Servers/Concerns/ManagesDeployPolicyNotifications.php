...
/**
 * Powers the "Notifications" sub-tab on the deploy-window workspace: binds
 * notification channels to this server's server.deploy_window.* events without
 * leaving the page.
 *
 * Deploy-window events fire from {@see \App\Jobs\RunSiteDeploymentJob} (blocked
 * deploys) and {@see \App\Livewire\Servers\WorkspaceDeployPolicy} (policy
 * toggled), so this trait is subscription management only. Mirrors
 * {@see ManagesCertInventoryNotifications}.
 */
trait ManagesDeployPolicyNotifications
{
    /** Channel selected in the add-subscription form on the Notifications tab. */
    public string $notif_channel_id = '';

    /**
     * server.deploy_window.* event keys ticked in the add form. Seeded to all of
    ...
    public function mountManagesDeployPolicyNotifications(): void
    {
        $this->notif_event_keys = ServerDeployPolicyNotificationKeys::eventKeys();
    }

    public function addDeployPolicyNotificationSubscription(): void
    {
        $this->authorize('update', $this->server);

        $allowedKeys = ServerDeployPolicyNotificationKeys::eventKeys();
        ...
        $assignable = $this->assignableDeployPolicyNotificationChannels()
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();
        ...
        $this->toastSuccess($created > 0
            ? __('Routing :count deploy window event(s) to :channel.', ['count' => $created, 'channel' => $channel->label])
            : __('Those events are already routed to :channel.', ['channel' => $channel->label]));
    }

    public function removeDeployPolicyNotificationSubscription(string $subscriptionId): void
    {
        $this->authorize('update', $this->server);
        ...
        $sub = NotificationSubscription::query()
            ...
            ->whereIn('event_key', ServerDeployPolicyNotificationKeys::eventKeys())
     ...
     */
    protected function assignableDeployPolicyNotificationChannels(): Collection
    {
     ...
     */
    protected function deployPolicyNotificationSubscriptions(): Collection
    {
     ...
     */
    protected function deployPolicyEventLabels(): array
    {
