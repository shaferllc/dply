...
/**
 * Powers the "Notifications" tab on the firewall workspace: binds notification
 * channels to this server's server.firewall_rule.* events without leaving the
 * page, and provides the {@see dispatchFirewallNotification()} helper the
 * component calls at each rule-mutation success point.
 *
 * Scoped to firewall keys only. Stakeholders already get in-app notifications
 * automatically; this tab routes them to email/Slack/webhook. Mirrors
 * {@see ManagesNetworkingNotifications}.
 */
trait ManagesFirewallNotifications
{
    ...
    {
        return ServerFirewallNotificationKeys::eventKeys();
    }
    ...
    /**
     * server.firewall_rule.* event keys ticked in the add form. Seeded to all of
    ...
    public function mountManagesFirewallNotifications(): void
    {
        $this->bootFeatureNotificationMatrix();

        $this->notif_event_keys = ServerFirewallNotificationKeys::eventKeys();
    }

    /**
     * Stable, human-readable label for a rule — its name, or an auto-label built
     * from action/protocol/port (matching the panel-event transcripts).
     */
    protected function firewallRuleLabel(ServerFirewallRule $rule): string
    {
        return $rule->name ?: ($rule->action.' '.$rule->protocol.' '.($rule->port ?? '*'));
    }

    /**
     * Fire a firewall notification for this server at a mutation success point.
     * The acting user is the current operator.
     *
     * @param  list<string>  $ruleLabels
     * @param  array<string, mixed>  $extraMetadata
     */
    protected function dispatchFirewallNotification(string $kind, array $ruleLabels = [], array $extraMetadata = []): void
    {
        app(ServerFirewallNotificationDispatcher::class)->notify(
            $this->server->fresh(),
            $kind,
            $ruleLabels,
            Auth::user(),
    ...
    public function addFirewallNotificationSubscription(): void
    {
        $this->authorize('update', $this->server);

        $allowedKeys = ServerFirewallNotificationKeys::eventKeys();
        ...
        $assignable = $this->assignableFirewallNotificationChannels()
            ->pluck('id')
            ->map(fn ($id) => (string) $id)
            ->all();
        ...
        $this->toastSuccess($created > 0
            ? __('Routing :count firewall event(s) to :channel.', ['count' => $created, 'channel' => $channel->label])
            : __('Those events are already routed to :channel.', ['channel' => $channel->label]));
    }

    public function removeFirewallNotificationSubscription(string $subscriptionId): void
    {
        $this->authorize('update', $this->server);
        ...
        $sub = NotificationSubscription::query()
            ...
            ->whereIn('event_key', ServerFirewallNotificationKeys::eventKeys())
     ...
     */
    protected function assignableFirewallNotificationChannels(): Collection
    {
     ...
     */
    protected function firewallNotificationSubscriptions(): Collection
    {
     ...
     */
    protected function firewallEventLabels(): array
    {
