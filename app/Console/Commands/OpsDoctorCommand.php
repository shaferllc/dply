...
{
    protected $signature = 'dply:ops:doctor
    ...
    /** Renamed from dply:fleet:doctor when the Fleet concept was dropped; old name still works. */
    protected $aliases = ['dply:fleet:doctor'];
    ...
    public function handle(): int
    {
        ...
        if ($totals['sites_with_failed_latest_deploy'] > 0) {
            $this->line('<fg=gray>Run </><fg=white>dply:ops:failed-deploys</><fg=gray> for the failure list.</>');
        }
        if ($totals['long_running_deploys'] > 0) {
            $this->line('<fg=gray>Run </><fg=white>dply:ops:running-deploys --older-than=15</><fg=gray> for stuck deploys.</>');
        }
