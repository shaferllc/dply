...
/**
 * Headless deploy of a container app onto the dply edge platform.
 * Mirrors the /edge/create UI as a CLI surface so operators can
 * automate from CI without clicking through the UI.
 *
 *   dply:edge:deploy <name> --image=<ref> [--port=8080] [--region=nyc]
 *                          [--backend=auto|digitalocean_app_platform|aws_app_runner]
 *                          [--user=<email>] [--org=<slug-or-id>]
 *
 * If <name> matches an existing edge site for the chosen org,
 * the command treats this as a redeploy — the new --image (if
 * different) is rolled via RedeployEdgeSiteJob.
 */
class EdgeDeployCommand extends Command
{
    protected $signature = 'dply:edge:deploy
    ...
    protected $description = 'Deploy or redeploy a container app on the dply edge platform.';
    ...
    public function handle(): int
    {
        ...
        if ($existing !== null) {
            ...
            RedeployEdgeSiteJob::dispatch($existing->id, $newImage);
            $this->info(sprintf(
