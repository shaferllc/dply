...
/**
 * Prints all cloud container sites in the fleet — the CLI mirror
 * of the /cloud index page. Useful for ops scripts and "what is
 * deployed where" sanity checks from the terminal.
 *
 *   dply:cloud:list [--json] [--backend=digitalocean_app_platform]
 *                  [--status=active|provisioning|failed]
 */
class CloudListCommand extends Command
{
    protected $signature = 'dply:cloud:list
        {--json : Output as JSON}
    ...
    protected $description = 'List cloud container sites across the fleet.';
    ...
    public function handle(): int
    {
        ...
        if ($rows === []) {
            $this->line('<fg=gray>No cloud sites found.</>');

            return self::SUCCESS;
        }

        $this->newLine();
        $this->line('<fg=cyan>Cloud container sites</> ('.count($rows).')');
        $this->newLine();
