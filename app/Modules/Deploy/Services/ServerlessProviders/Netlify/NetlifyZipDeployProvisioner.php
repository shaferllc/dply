...
namespace App\Modules\Deploy\Services\ServerlessProviders\Netlify;
...
final class NetlifyZipDeployProvisioner implements ServerlessFunctionProvisioner
{
    public function __construct(
        ...
        private readonly string $defaultSiteId,
    ...
    public function deployFunction(string $name, string $runtime, string $artifactPath, array $config = []): array
    {
        ...
        $response = Http::withToken($ctx['api_token'])
            ...
            ->post(sprintf('https://api.netlify.com/api/v1/sites/%s/deploys', rawurlencode($ctx['site_id'])));
    ...
    private function resolveNetlifyContext(array $config): array
    {
        ...
        if ($apiToken === '' || $siteId === '') {
            throw new RuntimeException('Netlify api_token and site_id are required (set NETLIFY_AUTH_TOKEN + NETLIFY_SITE_ID or project credentials / settings).');
        }
