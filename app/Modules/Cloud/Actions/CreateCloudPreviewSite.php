...
/**
 * Spawn a preview deployment for a source-mode cloud site.
 *
 * The "parent" Site is the production source-mode deploy (created by
 * CreateCloudSiteFromSource). A preview is a sibling Site that points
    ...
    public function handle(
        ...
        if (! is_array($parentSource) || ! is_string($parentSource['repo'] ?? null)) {
            throw new \RuntimeException(
                'Parent site has no source spec — only source-mode cloud sites can spawn previews.',
            );
        }
