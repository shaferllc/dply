...
/**
 * Roll an image-mode cloud site back to a previously deployed
 * image tag.
 *
 *   dply:cloud:rollback <site> [--image=<tag>] [--steps=N]
 *
 ...
 * git revert + dply:cloud:deploy for source rollbacks instead.
 */
class CloudRollbackCommand extends Command
{
    protected $signature = 'dply:cloud:rollback
        {site : Site ID, slug, or name}
        {--image= : Specific image tag to roll back to}
        {--steps=1 : How many history positions to roll back (default 1)}';

    protected $description = 'Roll an image-mode cloud site back to a previous image tag.';
    ...
    public function handle(): int
    {
        ...
        if (is_array($site->meta['container']['source'] ?? null)) {
            $this->error('Source-mode sites have no image history. Use git revert + dply:cloud:deploy to roll back.');
        ...
        RedeployCloudSiteJob::dispatch($site->id, $target);
