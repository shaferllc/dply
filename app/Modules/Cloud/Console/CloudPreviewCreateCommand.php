...
/**
 * Spawn a per-branch preview deployment for a source-mode cloud site.
 *
 *   dply:cloud:preview:create <parent> --branch=feature/x [--pr=42]
 *
 ...
 */
class CloudPreviewCreateCommand extends Command
{
    protected $signature = 'dply:cloud:preview:create
        {parent : Parent site ID, slug, or name}
        {--branch= : Git branch to deploy as the preview}
        {--pr= : Optional PR number for nicer naming (pr-42-…)}';

    protected $description = 'Spawn a preview deployment branch on the dply cloud platform.';
    ...
    public function handle(): int
    {
        ...
        try {
            $preview = (new CreateCloudPreviewSite)->handle($parent, $branch, $prNumber);
        } catch (\Throwable $e) {
