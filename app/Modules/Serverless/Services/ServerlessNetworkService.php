 ...
 * app's database and cache, and the servers (worker hosts, a jump box) that
 * need to reach them over private networking instead of the public internet.
 *
...
final class ServerlessNetworkService
{
    ...
    /**
     * Servers on the network — the boxes that can reach this app's clusters
     * privately, and from which an operator can open a psql/redis-cli session.
    ...
    /**
     * Whether either managed cluster was created before the app joined this
     * network. DigitalOcean cannot move an existing cluster between VPCs, so
     * the panel has to say so rather than imply a retro-fit.
     */
    public function hasClustersOutsideNetwork(Site $site): bool
    {
        $config = $site->serverlessConfig();
        $vpcUuid = $this->vpcUuid($site);

        if ($vpcUuid === null) {
            return false;
        }

        foreach (['database', 'cache'] as $key) {
            $resource = is_array($config[$key] ?? null) ? $config[$key] : [];
            if (($resource['status'] ?? '') === '' || empty($resource['cluster_id'])) {
                continue;
            }
            if ((string) ($resource['network_id'] ?? '') !== (string) ($config['network_id'] ?? '')) {
                return true;
            }
        }
