...
/**
 * DigitalOcean Managed Database operations for {@see DigitalOceanService}.
 *
 * Split out of the former ManagesDoFunctionsDatabases, which carried the DO
 * Functions namespace/trigger calls in the same trait. Deleting the serverless
 * surface (remove-cloud-edge-serverless) deleted the whole file and took the
 * managed-database half — a live capability, see app/Modules/Database — down
 * with it. This is that half, restored unchanged; only the Functions methods
 * are gone.
 */
trait ManagesDoDatabases
{
    /**
