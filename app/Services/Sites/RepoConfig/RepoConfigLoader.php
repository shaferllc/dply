...
namespace App\Services\Sites\RepoConfig;

use Symfony\Component\Yaml\Yaml;

/**
 * Parses an in-repo dply config file (YAML or JSON) into an
 * {@see RepoConfig} snapshot the build runner stores on the
 * deployment row.
 *
 * Schema (YAML form):
 *
...
class RepoConfigLoader
{
    ...
    public function loadFromDirectory(string $checkoutPath): ?RepoConfig
    {
     ...
     */
    public function parse(string $sourcePath, string $raw): RepoConfig
    {
        $warnings = [];
     ...
     * are accepted and merged at build time (see the build runner).
     *
     *   bindings:
     *     kv:
     ...
     * resource, resolved at deploy time.
     *
    ...
    private function normalizeBindings(mixed $value, array &$warnings): array
    {
        ...
        // time against the site's workspace (resolved at deploy time); the
    ...
    /**
     * Form endpoints (POST → email via control-plane ingest).
     *
     *   forms:
     *     enabled: true
     *     endpoints:
    ...
    private function normalizeCrons(mixed $value, array &$warnings): array
    {
        ...
        foreach ($value as $index => $entry) {
            if (count($out) >= 5) {
                $warnings[] = 'crons[]: up to 5 schedules per site are supported — extras ignored.';
                break;
            }
            if (! is_array($entry)) {
