...
class OrgScopedRedirectController extends Controller
{
    ...
    /** `/organizations/{org}/queues` → `/queues` */
    public function queues(Organization $organization): RedirectResponse
    {
        $this->switchTo($organization);

        return redirect()->route('queues.index', [], 301);
    }

    /** `/organizations/{org}/queues/{namespace}` → `/queues/{namespace}` */
    public function queueNamespace(Organization $organization, QueueNamespace $queueNamespace): RedirectResponse
    {
        $this->authorize('view', $organization);
        ...
        return redirect()->route('queues.show', ['queueNamespace' => $queueNamespace], 301);
    }
