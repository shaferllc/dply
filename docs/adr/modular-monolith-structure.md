...
- **Guard gap to close:** `LivewireAliasGuardTest` only covers Blade-embedded aliases, not full-page `Route::livewire` components. Add a test that asserts every full-page Livewire route component resolves by class at render time, so lesson (2) above can't recur silently.
