...
class SshConnectionCredentialSelectionTest extends TestCase
{
    ...
    {
        $connection = new class(new Server(['ssh_user' => 'deploy', 'ssh_private_key' => 'legacy-key', 'ssh_operational_private_key' => $this->validPrivateKey(), 'ssh_recovery_private_key' => null]), 'deploy', 'operational') extends SshConnection
        {
    ...
    {
        $connection = new class(new Server(['ssh_user' => 'deploy', 'ssh_private_key' => 'legacy-key', 'ssh_operational_private_key' => null, 'ssh_recovery_private_key' => $this->validPrivateKey()]), 'root', 'recovery') extends SshConnection
        {
            public function exposedPrivateKey(): ?string
            {
                return $this->privateKeyForConnection();
    ...
    {
        $connection = new class(new Server(['ssh_user' => 'deploy', 'ssh_private_key' => $this->validPrivateKey(), 'ssh_operational_private_key' => null, 'ssh_recovery_private_key' => null]), 'root', 'recovery') extends SshConnection
        {
            public function exposedPrivateKey(): ?string
            {
                return $this->privateKeyForConnection();
