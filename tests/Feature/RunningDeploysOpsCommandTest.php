...
namespace Tests\Feature\RunningDeploysOpsCommandTest;
...
test('lists currently running deploys', function () {
    ...
    // ignored
    Artisan::call('dply:ops:running-deploys', ['--json' => true]);
...
test('older than filter', function () {
    ...
    Artisan::call('dply:ops:running-deploys', [
        '--older-than' => 15,
        '--json' => true,
    ]);
    $decoded = json_decode(Artisan::output(), true);
...
test('friendly message when nothing running', function () {
    $exit = Artisan::call('dply:ops:running-deploys');
