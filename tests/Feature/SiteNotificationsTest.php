...
test('legacy webhooks settings url redirects to notifications', function () {
...
test('site settings mount redirects section webhooks to notifications', function () {
    $user = userWithOrganization();
    $org = $user->currentOrganization();
    $server = Server::factory()->ready()->create([
        'user_id' => $user->id,
...
test('site notifications section saves channel subscriptions', function () {
    $user = userWithOrganization();
    $org = $user->currentOrganization();
    $server = Server::factory()->ready()->create([
        'user_id' => $user->id,
...
test('uptime monitor check publishes down then recovered', function () {
    ...
    $downMeta = NotificationEvent::query()
        ...
        ->first()
        ?->metadata;
    expect($downMeta)->toBeArray();
    expect($downMeta['state'] ?? null)->toBe('down');
    ...
    expect($uptimeEvents[0]->metadata['state'] ?? null)->toBe('down');
    expect($uptimeEvents[1]->metadata['state'] ?? null)->toBe('recovered');
});
