...
namespace Tests\Feature\InfrastructureDeploysPageTest;
...
test('running tab shows in flight deploys', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.deploys'));
...
test('failed latest tab', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.deploys').'?tab=failed-latest');
...
test('stale tab with custom threshold', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.deploys').'?tab=stale&days=30');
...
test('friendly empty state per tab', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.deploys').'?tab=running');
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.deploys').'?tab=stale&days=7');
