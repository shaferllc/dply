...
namespace Tests\Feature\StaleDeploysOpsCommandTest;
...
test('lists sites with old deploys', function () {
    ...
    Artisan::call('dply:ops:stale-deploys', [
        '--days' => 30,
        '--json' => true,
    ]);
...
test('excludes never deployed by default', function () {
    ...
    Artisan::call('dply:ops:stale-deploys', ['--json' => true]);
    $decoded = json_decode(Artisan::output(), true);
...
test('human friendly message when nothing stale', function () {
    ...
    $exit = Artisan::call('dply:ops:stale-deploys');
