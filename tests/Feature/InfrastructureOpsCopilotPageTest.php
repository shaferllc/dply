...
namespace Tests\Feature\InfrastructureOpsCopilotPageTest;
...
usesFeatures('global.ops_copilot');
...
test('ops copilot page renders suggestions for failed deploy', function () {
    ...
    $this->actingAs($user)->get(route('infrastructure.copilot', ['site' => $site->id]))
        ->assertOk()
...
test('ops copilot route is hidden when flag is off', function () {
    ...
    $this->actingAs($user)->get(route('infrastructure.copilot'))
        ->assertStatus(400);
});
