...
class EdgeDomainCommandsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_attach_queues_job(): void
    {
        ...
        $exit = Artisan::call('dply:edge:domain:attach', [
            'site' => $site->name,
            'hostname' => 'api.example.com',
        ]);

        $this->assertSame(0, $exit);
        Queue::assertPushed(AttachEdgeDomainJob::class, fn ($j) => $j->siteId === $site->id && $j->hostname === 'api.example.com');
    }
    ...
    public function test_attach_normalizes_uppercase_and_strips_scheme(): void
    {
        ...
        Artisan::call('dply:edge:domain:attach', [
            'site' => $site->name,
            'hostname' => 'https://API.Example.COM/',
        ]);

        Queue::assertPushed(AttachEdgeDomainJob::class, fn ($j) => $j->hostname === 'api.example.com');
    }
    ...
    public function test_detach_queues_job(): void
    {
        ...
        $exit = Artisan::call('dply:edge:domain:detach', [
            'site' => $site->name,
            'hostname' => 'api.example.com',
        ]);

        $this->assertSame(0, $exit);
        Queue::assertPushed(DetachEdgeDomainJob::class, fn ($j) => $j->siteId === $site->id && $j->hostname === 'api.example.com');
    }
    ...
    public function test_list_emits_json_with_attached_domains(): void
    {
        ...
        Artisan::call('dply:edge:domain:list', ['site' => $site->name, '--json' => true]);
    ...
    public function test_list_empty_state(): void
    {
        ...
        $exit = Artisan::call('dply:edge:domain:list', ['site' => $site->name]);

        $this->assertSame(0, $exit);
        $this->assertStringContainsString('No custom domains attached', Artisan::output());
    }

    public function test_attach_rejects_non_edge_site(): void
    {
