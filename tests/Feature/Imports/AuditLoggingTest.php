...
/**
 * Migration lifecycle events get recorded in dply's AuditLog so an
 * operator can reconstruct what happened on a migration after the fact.
 */
class AuditLoggingTest extends TestCase
{
     ...
     */
    protected function seedMigration(string $status = ImportServerMigration::STATUS_STAGING): array
    {
    ...
    public function test_abort_writes_audit_log(): void
    {
        Bus::fake();
        [$user, $org, $migration] = $this->seedMigration();
    ...
    public function test_begin_cutover_writes_audit_log(): void
    {
        Bus::fake();
        [$user, $org, $migration, $site] = $this->seedMigration();
        ImportMigrationStep::create([
            'import_server_migration_id' => $migration->id,
        ...
        $this->assertSame('app.example.com', $log->new_values['domain']);
    }

    public function test_rollback_writes_audit_log(): void
    {
        Http::fake();
        [$user, $org, $migration, $site] = $this->seedMigration();
    ...
    public function test_retry_writes_audit_log(): void
    {
        Bus::fake();
        [$user, $org, $migration, $site] = $this->seedMigration();
    ...
    public function test_skip_writes_audit_log(): void
    {
        Bus::fake();
        [$user, $org, $migration, $site] = $this->seedMigration();
        $step = ImportMigrationStep::create([
    ...
    public function test_auto_expire_writes_audit_log(): void
    {
        Http::fake([
            'https://ploi.io/api/servers/42/keys/9001' => Http::response('', 204),
        ]);
        [, $org, $migration] = $this->seedMigration();
    ...
    public function test_kickoff_writes_audit_log_with_site_count(): void
    {
        Bus::fake();
        $user = User::factory()->create();
        ...
        $log = AuditLog::query()
            ...
            ->first();
        $this->assertNotNull($log);
        $this->assertSame('ploi', $log->new_values['source']);
        $this->assertSame(1, $log->new_values['site_count']);
    }
}
