...
namespace Tests\Feature\FailedDeploysOpsCommandTest;
...
test('lists sites with failed latest deploy', function () {
    ...
    $exit = Artisan::call('dply:ops:failed-deploys', ['--json' => true]);
...
test('skips running deploys by default', function () {
    ...
    Artisan::call('dply:ops:failed-deploys', ['--json' => true]);
    $decoded = json_decode(Artisan::output(), true);
...
test('include running treats running as latest', function () {
    ...
    Artisan::call('dply:ops:failed-deploys', [
...
test('zero failures returns success exit code', function () {
    ...
    $exit = Artisan::call('dply:ops:failed-deploys');
