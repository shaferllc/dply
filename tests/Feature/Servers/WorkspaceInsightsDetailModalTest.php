...
function makeFinding(Server $server, array $overrides = []): InsightFinding
{
...
test('opening detail loads the finding and decorates actions', function () {
    [$user, $server] = userServerOrg();
    $finding = makeFinding($server);
    ...
    expect($detail['finding']->id)->toBe($finding->id);
    expect($detail['fixHistory']['run_status'])->toBe('idle');
    expect($detail['actions']['canRunFix'])->toBeTrue();
    expect($detail['actions']['canRerun'])->toBeTrue();
    expect($detail['actions']['canAcknowledge'])->toBeTrue();
    expect($detail['actions']['canIgnore'])->toBeFalse();
    expect($detail['signalRows'])->toBe(['security_count' => 96, 'upgradable_count' => 154]);
});
test('opening detail ignores findings belonging to another server', function () {
    [$user, $server] = userServerOrg();
    ...
    $strayFinding = makeFinding($anotherServer);
...
test('acknowledge action closes the detail modal when it targets the open finding', function () {
    [$user, $server] = userServerOrg();
    $finding = makeFinding($server);
...
test('run fix dispatches job and stamps run started at', function () {
    ...
    $finding->refresh();
    expect($finding->meta)->toHaveKey('fix_run_started_at');
    expect($finding->meta['fix_run_started_by'])->toBe($user->id);
});
test('run fix clears prior terminal meta so status flips back to queued', function () {
    Bus::fake([ApplyInsightFixJob::class]);
    ...
    expect($detailBefore['fixHistory']['run_status'])->toBe('failed');
...
test('run fix is a noop when no fix handler is configured', function () {
    Bus::fake([ApplyInsightFixJob::class]);
...
test('correlation pivot loads a sibling finding', function () {
    [$user, $server] = userServerOrg();
    $first = makeFinding($server, ['title' => 'First finding']);
    $second = makeFinding($server, ['title' => 'Second finding']);
    ...
    $detail = $component->instance()->selectedFindingDetail;
    expect($detail['correlationFindings'])->toHaveCount(1);
    expect($detail['correlationFindings']->first()->id)->toBe($second->id);
