...
namespace Tests\Feature\InfrastructureEnvSearchPageTest;
...
test('landing state has no results until query', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search'));

    $response->assertOk()
        ->assertSee('Env search')
...
test('finds exact key across sites', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search').'?q=DATABASE_URL');
...
test('prefix mode', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search').'?q=AWS_&mode=prefix');
...
test('values masked by default', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search').'?q=API_KEY');
...
test('no match message', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search').'?q=NOPE');

    $response->assertOk()
        ->assertSee('No matches across the organization');
});
test('only searches within current org', function () {
    ...
    $response = $this->actingAs($user)->get(route('infrastructure.env-search').'?q=CROSS_ORG_KEY');
