...
final class CloudProjectController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $perPage = min(max((int) $request->query('per_page', 25), 1), 100);

        $query = CloudProject::query()->orderBy('name');
        ...
        return response()->json([
            'data' => collect($paginator->items())->map(fn (CloudProject $p) => $this->projectPayload($p))->values()->all(),
            'meta' => [
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'slug' => ['required', 'string', 'max:128', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/', 'unique:cloud_projects,slug'],
            'settings' => ['sometimes', 'array'],
            'credentials' => ['sometimes', 'array'],
        ]);

        $project = CloudProject::query()->create($validated);

        return response()->json(['project' => $this->projectPayload($project)], 201);
    }

    public function update(Request $request, CloudProject $project): JsonResponse
    {
        $validated = $request->validate([
    ...
    public function show(CloudProject $project): JsonResponse
    {
        ...
        return response()->json([
            ...
            'latest_deployment' => $latest === null ? null : [
                'id' => $latest->id,
                'deployment_url' => route('cloud.deployments.show', $latest, absolute: true),
     ...
     */
    private function projectPayload(CloudProject $project): array
    {
        return [
            'id' => $project->id,
            'name' => $project->name,
