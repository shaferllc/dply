...
final class CloudDeployRequestParser
{
    /**
     * @return array{cloud_project_id: int, application_name: string, stack: string, git_ref: string, idempotency_key: string|null}
     */
    public function parse(Request $request): array
    {
        ...
        $defaultStack = (string) config('cloud.default_stack', 'php');
        $defaultGitRef = (string) config('cloud.default_git_ref', 'main');

        $stack = isset($data['stack']) ? trim((string) $data['stack']) : $defaultStack;
        if ($stack === '') {
            $stack = $defaultStack;
        }

        $allowedStacks = ['php', 'rails'];
        if (! in_array($stack, $allowedStacks, true)) {
            throw new InvalidArgumentException('stack must be one of: '.implode(', ', $allowedStacks).'.');
        }
