...
final class WordpressProjectController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $query = WordpressProject::query()->orderBy('name');
        ...
        return response()->json([
            'data' => collect($paginator->items())->map(fn (WordpressProject $p) => $this->projectPayload($p))->values()->all(),
            'meta' => [
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'slug' => ['required', 'string', 'max:128', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/', 'unique:wordpress_projects,slug'],
    ...
    public function update(Request $request, WordpressProject $project): JsonResponse
    {
        $validated = $request->validate([
    ...
    public function show(WordpressProject $project): JsonResponse
    {
        ...
        return response()->json([
            ...
            'latest_deployment' => $latest === null ? null : [
                'id' => $latest->id,
                'deployment_url' => route('wordpress.deployments.show', $latest, absolute: true),
                'status' => $latest->status,
                'application_name' => $latest->application_name,
                'php_version' => $latest->php_version,
     ...
     */
    private function projectPayload(WordpressProject $project): array
    {
        return [
            'id' => $project->id,
            'name' => $project->name,
