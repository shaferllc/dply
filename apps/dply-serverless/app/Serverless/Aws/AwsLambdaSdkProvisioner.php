...
/**
 * AWS SDK provisioner: optional S3 {@see AwsLambdaGateway::updateFunctionCodeFromS3} when
 * {@see $s3AllowBuckets} is non-empty and the artifact path is an s3://bucket/key URI (optionally narrowed
 * per project via `settings.aws_s3_allow_buckets`); optional
 * local zip via {@see AwsLambdaGateway::updateFunctionCodeWithZip} (paths under {@see $zipPathPrefix});
 * otherwise {@see AwsLambdaGateway::describeFunction} only.
 */
final class AwsLambdaSdkProvisioner implements ServerlessFunctionProvisioner
{
    ...
    public function __construct(
        private AwsLambdaGateway $gateway,
        private string $defaultAwsRegion,
        private bool $uploadZipWhenFileExists,
        private ?string $zipPathPrefix,
        private int $zipMaxBytes,
        private array $s3AllowBuckets = [],
    ) {}
    ...
    /**
     * Effective allow list: global {@see $s3AllowBuckets} when the project omits the
     * `aws_s3_allow_buckets` setting; otherwise the intersection (case-insensitive) with the project list.
     * An explicit empty project list disallows all S3 artifacts for that deploy.
     *
    ...
    private function tryDeployFromS3(AwsLambdaGateway $gateway, string $functionName, string $artifactPath, array $config): ?array
    {
        ...
        if ($allowedBuckets === []) {
            if ($this->s3AllowBuckets === []) {
                throw new RuntimeException(
                    'artifact_path uses s3:// but no buckets are allowed. Set SERVERLESS_AWS_S3_ALLOW_BUCKETS (comma-separated).'
                );
            }

            throw new RuntimeException(
                'S3 artifact is not allowed for this project: set project settings aws_s3_allow_buckets to a non-empty list that overlaps SERVERLESS_AWS_S3_ALLOW_BUCKETS, or omit the key to inherit the global list.'
            );
        }
        ...
        if (! in_array(strtolower($s3['bucket']), $allowedLower, true)) {
            throw new RuntimeException(
                'S3 bucket "'.$s3['bucket'].'" is not allowed for this deploy (global allow list and/or project aws_s3_allow_buckets).'
            );
        }
    ...
    {
        if (! $this->uploadZipWhenFileExists) {
            return false;
        }
        if ($this->zipPathPrefix === null || $this->zipPathPrefix === '') {
            return false;
        }
