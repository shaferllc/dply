...
namespace App\Serverless\Vercel;
...
/**
 * Expands a local zip (under a path prefix) and creates a Vercel deployment via POST /v13/deployments.
 *
 * @see https://vercel.com/docs/rest-api/reference/endpoints/deployments/create-a-new-deployment
 */
final class VercelZipDeployProvisioner implements ServerlessFunctionProvisioner
{
    /**
     * @param  array<int, string>  $ignoredZipPathPrefixes  Path prefixes (forward slashes) to skip inside the zip, e.g. __MACOSX/
     */
    public function __construct(
        ...
        private array $ignoredZipPathPrefixes = ['__MACOSX/'],
    ) {}
    ...
    private function resolveVercelContext(array $config): array
    {
        ...
        if ($token === '') {
            throw new RuntimeException(
                'Vercel token is required (set VERCEL_TOKEN or project credentials vercel_token / api_token).'
            );
        }

        if ($projectId === '' && $projectName === '') {
            throw new RuntimeException(
                'Vercel project id or project name is required (set VERCEL_PROJECT_ID or VERCEL_PROJECT_NAME, or project settings / credentials).'
            );
        }
