...
namespace App\Serverless\Cloudflare;
...
/**
 * Uploads a Worker script via the Cloudflare API (multipart upload).
 *
...
final class CloudflareWorkersProvisioner implements ServerlessFunctionProvisioner
{
    ...
    private function resolveCloudflareContext(array $config): array
    {
        ...
        $compat = trim((string) ($settings['cloudflare_compatibility_date'] ?? ''));
        if ($compat === '') {
            $compat = $this->compatibilityDate;
        }

        if ($accountId === '' || $apiToken === '') {
            throw new RuntimeException(
                'Cloudflare account_id and api_token are required (set CLOUDFLARE_ACCOUNT_ID / CLOUDFLARE_API_TOKEN or project credentials / settings).'
            );
        }
    ...
    {
        $s = strtolower((string) preg_replace('/[^a-zA-Z0-9_-]+/', '-', $name));
        $s = trim($s, '-');
        if ($s === '' || strlen($s) > 255) {
